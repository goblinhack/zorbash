...
std::ostream &operator<<(std::ostream &out, Bits< MonstInfop & > const my)
{
  ...
  /* int          enchant                   */ out << bits(my.t->enchant_max);
  /* int          gold                      */ out << bits(my.t->gold);
  /* int          health                    */ out << bits(my.t->health);
  /* int          health                    */ out << bits(my.t->health_max);
  ...
  /* int          monst_state               */ out << bits(my.t->monst_state);
  /* int          stamina                   */ out << bits(my.t->stamina);
  /* int          stamina                   */ out << bits(my.t->stamina_max);
  ...
  /* ts_t  ts_fadeup_end                    */ out << bits(my.t->ts_fadeup_end);
  /* ts_t  ts_fall_begin                    */ out << bits(my.t->ts_fall_begin);
  /* ts_t  ts_fall_end                      */ out << bits(my.t->ts_fall_end);
  /* ts_t  ts_flip_start                    */ out << bits(my.t->ts_flip_start);
  /* ts_t  ts_lunge_begin                   */ out << bits(my.t->ts_lunge_begin);
  /* ts_t  ts_lunge_end                     */ out << bits(my.t->ts_lunge_end);
  /* ts_t  ts_anim_delay_end                */ out << bits(my.t->ts_anim_delay_end);
  /* uint32_t     tick_last_did_something   */ out << bits(my.t->tick_last_did_something);
  /* uint32_t     tick_last_dropped         */ out << bits(my.t->tick_last_dropped);
...
std::ostream &operator<<(std::ostream &out, Bits< Levelp & > const my)
{
  ...
  out << bits(my.t->ts_dungeon_created);
  ts_t ts_dungeon_saved = time_get_time_ms();
  out << bits(ts_dungeon_saved);
  out << bits(my.t->ts_fade_out_begin);
  out << bits(my.t->ts_fade_in_begin);

  /* array<array<bool, MAP_HEIGHT>, MAP_WIDTH> _is_light_blocker {};       */ out << bits(my.t->_is_light_blocker);
  /* array<array<bool, MAP_HEIGHT>, MAP_WIDTH> _is_lit_ever {};            */ out << bits(my.t->_is_lit_ever);
  /* array<array<bool, MAP_HEIGHT>, MAP_WIDTH> _is_obs_wall_or_door {};    */ out << bits(my.t->_is_obs_wall_or_door);
  /* array<array<bool, MAP_HEIGHT>, MAP_WIDTH> _is_obs_destructable {};    */ out << bits(my.t->_is_obs_destructable);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _fade_in_map {};         */ out << bits(my.t->_fade_in_map);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _gfx_water {};           */ out << bits(my.t->_gfx_water);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _heatmap {};             */ out << bits(my.t->_heatmap);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_able_to_stand_on {}; */ out << bits(my.t->_is_able_to_stand_on);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_acid {};             */ out << bits(my.t->_is_acid);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_ascend_dungeon {};   */ out << bits(my.t->_is_ascend_dungeon);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_ascend_sewer {};     */ out << bits(my.t->_is_ascend_sewer);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_barrel {};           */ out << bits(my.t->_is_barrel);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_blood {};            */ out << bits(my.t->_is_blood);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_brazier {};          */ out << bits(my.t->_is_brazier);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_bridge {};           */ out << bits(my.t->_is_bridge);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_chasm {};            */ out << bits(my.t->_is_chasm);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_corpse {};           */ out << bits(my.t->_is_corpse);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_corridor {};         */ out << bits(my.t->_is_corridor);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_deep_water {};       */ out << bits(my.t->_is_deep_water);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_descend_dungeon {};  */ out << bits(my.t->_is_descend_dungeon);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_descend_sewer {};    */ out << bits(my.t->_is_descend_sewer);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_dirt {};             */ out << bits(my.t->_is_dirt);
  ...
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_food {};             */ out << bits(my.t->_is_food);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_gold {};             */ out << bits(my.t->_is_gold);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_hazard {};           */ out << bits(my.t->_is_hazard);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_key {};              */ out << bits(my.t->_is_key);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_lava {};             */ out << bits(my.t->_is_lava);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_lit_currently {};    */ out << bits(my.t->_is_lit_currently);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_minion_generator {}; */ out << bits(my.t->_is_minion_generator);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_monst {};            */ out << bits(my.t->_is_monst);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_poison {};           */ out << bits(my.t->_is_poison);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_potion {};           */ out << bits(my.t->_is_potion);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_ripple {};           */ out << bits(my.t->_is_ripple);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_rock {};             */ out << bits(my.t->_is_rock);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_secret_door {};      */ out << bits(my.t->_is_secret_door);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_shallow_water {};    */ out << bits(my.t->_is_shallow_water);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_skillstone {};       */ out << bits(my.t->_is_skillstone);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_smoke {};            */ out << bits(my.t->_is_smoke);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_spiderweb {};        */ out << bits(my.t->_is_spiderweb);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_sticky {};           */ out << bits(my.t->_is_sticky);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_shovable {};         */ out << bits(my.t->_is_shovable);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_treasure_class_a {}; */ out << bits(my.t->_is_treasure_class_a);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_treasure_class_b {}; */ out << bits(my.t->_is_treasure_class_b);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_treasure_class_c {}; */ out << bits(my.t->_is_treasure_class_c);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_treasure_type {};    */ out << bits(my.t->_is_treasure_type);
  /* array<array<uint8_t, MAP_HEIGHT>, MAP_WIDTH> _is_wall {};             */ out << bits(my.t->_is_wall);
