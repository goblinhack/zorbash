...
PyObject *map_load_skill_tree_(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  static char *kwlist[] = {(char *) "skill_data", (char *) "tree_data", nullptr};

  if (! PyArg_ParseTupleAndKeywords(args, keywds, "|OO", kwlist, &py_skill_data, &py_tree_data)) {
    DIE("map_load_skill_tree: Bad args");
    Py_RETURN_FALSE;
  }

  if (! py_skill_data) {
    DIE("map_load_skill_tree: missing skill data");
    Py_RETURN_FALSE;
  }

  if (! py_tree_data) {
    DIE("map_load_skill_tree: missing tree data");
    Py_RETURN_FALSE;
  }
  ...
  for (auto y = 0; y < SKILL_TREE_DOWN * 2; y++) {
    ...
    for (auto x = 0; x < SKILL_TREE_ACROSS * 4; x++) {
      ...
      if (connector_string == "<-") {
        TRACE_NO_INDENT();
        auto skill_src = get(game->skill_tree, skill_x + 1, skill_y);
        if (! skill_src) {
        ...
        TRACE_NO_INDENT();
        auto skill_dst = get(game->skill_tree, skill_x, skill_y);
        if (! skill_dst) {
      ...
      if (connector_string == "->") {
        TRACE_NO_INDENT();
        auto skill_src = get(game->skill_tree, skill_x, skill_y);
        if (! skill_src) {
          DIE("Have -> connector at %u,%u, but no corresponding source skill index %u, %u", x, y, skill_x, skill_y);
        }

        TRACE_NO_INDENT();
        auto skill_dst = get(game->skill_tree, skill_x + 1, skill_y);
        if (! skill_dst) {
          DIE("Have -> connector at %u,%u, but no corresponding dest skill index %u, %u", x, y, skill_x + 1, skill_y);
        }
      ...
      if (connector_string == "v ") {
        ...
        auto skill_src = get(game->skill_tree, skill_x, skill_y);
        if (! skill_src) {
          DIE("Have down connector at %u,%u, but no corresponding source skill index %u, %u", x, y, skill_x, skill_y);
        }

        TRACE_NO_INDENT();
        auto skill_dst = get(game->skill_tree, skill_x, skill_y + 1);
        if (! skill_dst) {
      ...
      if (connector_string == "^ ") {
        TRACE_NO_INDENT();
        auto skill_src = get(game->skill_tree, skill_x, skill_y + 1);
        if (! skill_src) {
  ...
  for (auto y = 0; y < SKILL_TREE_DOWN; y++) {
    for (auto x = 0; x < SKILL_TREE_ACROSS; x++) {
      ...
      if (skill->skill_right) {
        Skillp skill_right = get(game->skill_tree, x + 1, y);
        if (skill_right) {
          TRACE_NO_INDENT();
      ...
      TRACE_NO_INDENT();
      if (skill->skill_left) {
        Skillp skill_left = get(game->skill_tree, x - 1, y);
        if (skill_left) {
          TRACE_NO_INDENT();
      ...
      TRACE_NO_INDENT();
      if (skill->skill_down) {
        Skillp skill_down = get(game->skill_tree, x, y + 1);
        if (skill_down) {
          TRACE_NO_INDENT();
      ...
      TRACE_NO_INDENT();
      if (skill->skill_up) {
        Skillp skill_up = get(game->skill_tree, x, y - 1);
        if (skill_up) {
          TRACE_NO_INDENT();
