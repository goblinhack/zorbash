...
const std::string Thing::dmg_digest_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_digest_dice_str() + modifier_to_string(enchant_count_get());
  }

  return (tp()->dmg_digest_dice_str());
}

int Thing::dmg_digest(void)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_receive_dmg_digest(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_owner_receive_dmg_digest)) {
    return damage;
  }
...
int Thing::total_dmg_for_on_receiving_dmg_digest(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_receive_dmg_digest(this, hitter, real_hitter, damage);
    }
  }
...
int Thing::dmg_digest_chance_d1000(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_digest_chance_d1000(index));
}
...
int Tp::dmg_digest_chance_d1000(int index) const
{
  if (index >= (int) _dmg_digest_chance_d1000.size()) {
    return 0;
  }
  return _dmg_digest_chance_d1000[ index ];
}

void Tp::dmg_digest_chance_d1000_set(int index, int v)
{
  _dmg_digest_chance_d1000.resize(index + 1);
  _dmg_digest_chance_d1000[ index ] = v;
}
