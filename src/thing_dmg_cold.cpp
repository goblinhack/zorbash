...
const std::string Thing::dmg_cold_dice_str(void)
{
  ...
  if (enchant_get()) {
    return tp()->dmg_cold_dice_str() + modifier_to_string(enchant_get());
  }

  return (tp()->dmg_cold_dice_str());
}

int Thing::dmg_cold(void)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_receive_dmg_cold(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_receive_dmg_cold = on_owner_receive_dmg_cold_do();
  if (std::empty(on_owner_receive_dmg_cold)) {
    return damage;
  }

  auto t = split_tokens(on_owner_receive_dmg_cold, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_receive_dmg_cold call [%s] expected mod:function, got %d elems",
      on_owner_receive_dmg_cold.c_str(), (int) on_owner_receive_dmg_cold.size());

  return damage;
}

int Thing::on_receiving_dmg_cold(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_receiving_dmg_cold = on_receiving_dmg_cold_do();
  if (std::empty(on_receiving_dmg_cold)) {
    return damage;
  }

  auto t = split_tokens(on_receiving_dmg_cold, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_receiving_dmg_cold call [%s] expected mod:function, got %d elems", on_receiving_dmg_cold.c_str(),
      (int) on_receiving_dmg_cold.size());

  return damage;
}

int Thing::total_dmg_for_on_receiving_dmg_cold(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_receive_dmg_cold(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_receiving_dmg_cold(hitter, real_hitter, damage);
...
int Thing::dmg_cold_chance_d1000(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_cold_chance_d1000(index));
}
...
const std::string &Thing::on_owner_receive_dmg_cold_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_receive_dmg_cold_do());
}

int Thing::dmg_received_doubled_from_cold(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_received_doubled_from_cold());
}

int Tp::dmg_cold_chance_d1000(int index) const
{
  if (index >= (int) _dmg_cold_chance_d1000.size()) {
    return 0;
  }
  return _dmg_cold_chance_d1000[ index ];
}

void Tp::dmg_cold_chance_d1000_set(int index, int v)
{
  _dmg_cold_chance_d1000.resize(index + 1);
  _dmg_cold_chance_d1000[ index ] = v;
}
