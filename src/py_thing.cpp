...
PyObject *thing_coords_get(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! the_tuple) {
    PY_ERR("%s: Cannot create tuple for ID %u", __FUNCTION__, id);
    Py_RETURN_NONE;
  }

  auto err = PyTuple_SetItem(the_tuple, (Py_ssize_t) 0, PyLong_FromLong((long) t->curr_at.x));
  if (err < 0) {
    PY_ERR("%s: Cannot create tuple (a) for ID %u", __FUNCTION__, id);
    Py_RETURN_NONE;
  }
...
PyObject *thing_possible_to_attack(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! hitter_id) {
    PY_ERR("%s: No hitter thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_shoot_projectile_or_laser_at(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! owner_id) {
    PY_ERR("%s: No owner thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  Thingp owner = game->thing_find(owner_id);
  if (! owner) {
    PY_ERR("%s: Cannot find owner thing ID %u", __FUNCTION__, owner_id);
    Py_RETURN_FALSE;
  }

  if (! item) {
    PY_ERR("%s: No item thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
  ...
  if (! tp_cands.size()) {
    PY_ERR("Cannot find any %s to spawn", what.c_str());
    Py_RETURN_FALSE;
  }

  auto itemp = pcg_one_of(tp_cands);
  if (! itemp) {
    PY_ERR("%s: Cannot find item to shoot %s", __FUNCTION__, item);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_throw_at(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! tp_cands.size()) {
    PY_ERR("Cannot find any %s to spawn", item);
    Py_RETURN_FALSE;
  }

  auto tp = pcg_one_of(tp_cands);
  if (unlikely(! tp)) {
    PY_ERR("Could not find to throw %s", item);
    Py_RETURN_FALSE;
  }

  auto what = owner->level->thing_new(tp, target->curr_at);
  if (! what) {
    PY_ERR("Could not create to throw %s", item);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_spell_cast_at(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  auto tp = pcg_one_of(tp_cands);
  if (unlikely(! tp)) {
    PY_ERR("Could not find to spell_cast %s", item);
    Py_RETURN_FALSE;
  }

  auto what = owner->level->thing_new(tp, target->curr_at);
  if (! what) {
    PY_ERR("Could not create to spell_cast %s", item);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_death_by(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! id) {
    PY_ERR("%s: No t thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  Thingp t = game->thing_find(id);
  if (! t) {
    PY_ERR("%s: Cannot find t thing ID %u", __FUNCTION__, id);
    Py_RETURN_FALSE;
  }

  if (! reason) {
    PY_ERR("%s: No reason thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  if (! killer_id) {
    PY_ERR("%s: No defeater thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  Thingp defeater = game->thing_find(killer_id);
  if (! defeater) {
    PY_ERR("%s: Cannot find defeater thing ID %u", __FUNCTION__, killer_id);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_beckon(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! me_id) {
    PY_ERR("%s: No me_id thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  if (! it_id) {
    PY_ERR("%s: No it_id thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  Thingp me = game->thing_find(me_id);
  if (! me) {
    PY_ERR("%s: Cannot find me thing ID %u", __FUNCTION__, me_id);
    Py_RETURN_FALSE;
  }

  Thingp it = game->thing_find(it_id);
  if (! it) {
    PY_ERR("%s: Cannot find it thing ID %u", __FUNCTION__, it_id);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_friend_add(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! it_id) {
    PY_ERR("%s: No friend thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
  ...
  Thingp it = game->thing_find(it_id);
  if (! it) {
    PY_ERR("%s: Cannot find friend thing ID %u", __FUNCTION__, id);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_polymorph(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  auto tp    = pcg_one_of(cands);
  if (unlikely(! tp)) {
    PY_ERR("%s: No polymorph into %s found", __FUNCTION__, into);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_buff_add(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! what) {
    PY_ERR("%s: No buff name set", __FUNCTION__);
    Py_RETURN_FALSE;
  }

  auto tp_cands = tp_find_wildcard(what);
  if (! tp_cands.size()) {
    PY_ERR("Cannot find any %s to spawn", what);
    Py_RETURN_FALSE;
  }

  auto tp = pcg_one_of(tp_cands);
  if (unlikely(! tp)) {
    PY_ERR("%s: No buff tp called %s found", __FUNCTION__, what);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_debuff_add(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! what) {
    PY_ERR("%s: No debuff name set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
  ...
  auto tp = pcg_one_of(tp_cands);
  if (unlikely(! tp)) {
    PY_ERR("%s: No debuff tp called %s found", __FUNCTION__, what);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_speaks(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! msg) {
    PY_ERR("%s: No msg thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_popup(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! msg) {
    PY_ERR("%s: No popup msg thing ID set", __FUNCTION__);
    Py_RETURN_FALSE;
  }
...
PyObject *thing_sound_play_(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! PyArg_ParseTupleAndKeywords(args, keywds, "Is", kwlist, &owner_id, &name)) {
    PY_ERR("thing_sound_play: Bad arguments");
    Py_RETURN_FALSE;
  }
  ...
  if (! name) {
    PY_ERR("thing_sound_play: Missing name attr");
    Py_RETURN_FALSE;
  }
...
PyObject *thing_sound_play_channel_(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (! PyArg_ParseTupleAndKeywords(args, keywds, "Iis", kwlist, &owner_id, &channel, &name)) {
    PY_ERR("thing_sound_play_channel: Bad arguments");
    Py_RETURN_FALSE;
  }
  ...
  if (! name) {
    PY_ERR("thing_sound_play_channel: Missing name attr");
    Py_RETURN_FALSE;
  }
...
PyObject *spawn_gas_paralysis_around_thing(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (t->spawn_gas_paralysis_around_thing(radius)) {
    Py_RETURN_TRUE;
  }
  Py_RETURN_FALSE;
}
...
PyObject *spawn_gas_confusion_around_thing(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (t->spawn_gas_confusion_around_thing(radius)) {
    Py_RETURN_TRUE;
  }
  Py_RETURN_FALSE;
}
...
PyObject *spawn_darkness_around_thing(PyObject *obj, PyObject *args, PyObject *keywds)
{
  ...
  if (t->spawn_darkness_around_thing(radius)) {
    Py_RETURN_TRUE;
  }
  Py_RETURN_FALSE;
}
