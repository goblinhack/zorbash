...
const std::string Thing::dmg_poison_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_poison_dice_str() + bonus_to_string(enchant_count_get());
  }

  return (tp()->dmg_poison_dice_str());
}

int Thing::dmg_poison(Thingp victim)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_rcv_dmg_poison(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_rcv_dmg_poison = on_owner_rcv_dmg_poison_do();
  if (std::empty(on_owner_rcv_dmg_poison)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_poison, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_poison call [%s] expected mod:function, got %d elems", on_owner_rcv_dmg_poison.c_str(),
      (int) on_owner_rcv_dmg_poison.size());

  return damage;
}

int Thing::on_rcv_dmg_poison(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_rcv_dmg_poison = on_rcv_dmg_poison_do();
  if (std::empty(on_rcv_dmg_poison)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_poison, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_poison call [%s] expected mod:function, got %d elems", on_rcv_dmg_poison.c_str(),
      (int) on_rcv_dmg_poison.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_poison(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_poison(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_poison(hitter, real_hitter, damage);
...
const std::string &Thing::on_rcv_dmg_poison_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_poison_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_poison_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_poison_do());
}

int Thing::dmg_rcv_doubled_from_poison(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_rcv_doubled_from_poison());
}
