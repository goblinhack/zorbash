...
void Level::place_objects_with_normal_placement_rules(Dungeonp d)
{
  ...
  for (auto x = MAP_BORDER_ROCK; x < MAP_WIDTH - MAP_BORDER_ROCK; x++) {
    for (auto y = MAP_BORDER_ROCK; y < MAP_HEIGHT - MAP_BORDER_ROCK; y++) {
      ...
      if (d->is_red_blood(x, y)) { tp = tp_random_red_blood(); }
      ...
      if (d->is_food(x, y)) {
        if (d1000() < 500) { tp = tp_random_food(); }
      }
      if (d->is_gold(x, y)) {
        if (d1000() < 500) { tp = tp_random_gold(); }
      }
      if (d->is_key(x, y)) { tp = tp_random_key(); }
      if (d->is_potion(x, y)) {
        if (d1000() < 500) { tp = tp_random_potion(); }
      }
      if (d->is_staff(x, y)) {
        if (d1000() < 500) { tp = tp_random_staff(); }
      }
      if (d->is_ring(x, y)) {
        if (d1000() < 500) { tp = tp_random_ring(); }
      }
      ...
      if (d->is_mob_challenge_class_A(x, y) || d->is_mob_challenge_class_B(x, y)) {
        ...
        if (d->is_hazard(x - 1, y) && d->is_hazard(x + 1, y) && d->is_hazard(x, y - 1) && d->is_hazard(x, y + 1)
          ...
          if (d1000() < d1000_chance_creating_mob_challenge_class_A) { tp = tp_random_ethereal_mob(); }
        } else {
          //
          // Else choose a normal mob
          //
          if (d->is_mob_challenge_class_A(x, y)) {
            if (d1000() < d1000_chance_creating_mob_challenge_class_A) { tp = tp_random_mob_challenge_class_A(p); }
          }
          if (d->is_mob_challenge_class_B(x, y)) {
            if (d1000() < d1000_chance_creating_mob_challenge_class_B) { tp = tp_random_mob_challenge_class_B(p); }
          }
        }
      }

      if (d->is_trap(x, y)) {
        if (d1000() < 50) { tp = tp_random_trap(); }
      }

      if (d->is_barrel(x, y)) {
        if (d1000() < 200) { tp = tp_random_barrel(); }
      }
      ...
      if (r && r->is_secret) {
        ...
        if (d->is_treasure_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_A * 3) { tp = tp_random_treasure_class_A(p); }
        }
        if (d->is_treasure_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_B * 3) { tp = tp_random_treasure_class_B(p); }
        }
        if (d->is_treasure_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_C * 3) { tp = tp_random_treasure_class_C(p); }
        }
      } else if (is_more_generous_room) {
        if (d->is_treasure_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_A * 2) { tp = tp_random_treasure_class_A(p); }
        }
        if (d->is_treasure_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_B * 2) { tp = tp_random_treasure_class_B(p); }
        }
        if (d->is_treasure_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_C * 2) { tp = tp_random_treasure_class_C(p); }
        }
      } else {
        if (d->is_treasure_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_A) { tp = tp_random_treasure_class_A(p); }
        }
        if (d->is_treasure_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_B) { tp = tp_random_treasure_class_B(p); }
        }
        if (d->is_treasure_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_treasure_class_C) { tp = tp_random_treasure_class_C(p); }
        }
      }
      ...
      if (r && r->is_secret) {
        ...
        if (d->is_weapon_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_A * 3) { tp = tp_random_weapon_class_A(p); }
        }
        if (d->is_weapon_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_B * 3) { tp = tp_random_weapon_class_B(p); }
        }
        if (d->is_weapon_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_C * 3) { tp = tp_random_weapon_class_C(p); }
        }
      } else if (is_more_generous_room) {
        if (d->is_weapon_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_A * 2) { tp = tp_random_weapon_class_A(p); }
        }
        if (d->is_weapon_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_B * 2) { tp = tp_random_weapon_class_B(p); }
        }
        if (d->is_weapon_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_C * 2) { tp = tp_random_weapon_class_C(p); }
        }
      } else {
        if (d->is_weapon_class_A(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_A) { tp = tp_random_weapon_class_A(p); }
        }
        if (d->is_weapon_class_B(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_B) { tp = tp_random_weapon_class_B(p); }
        }
        if (d->is_weapon_class_C(x, y)) {
          if (d1000() < d1000_chance_creating_weapon_class_C) { tp = tp_random_weapon_class_C(p); }
        }
      }

      if (d->is_magic_stone(x, y)) { tp = tp_random_magic_stone(); }
  ...
  for (auto x = MAP_BORDER_ROCK; x < MAP_WIDTH - MAP_BORDER_ROCK; x++) {
    for (auto y = MAP_BORDER_ROCK; y < MAP_HEIGHT - MAP_BORDER_ROCK; y++) {
      if (! d->is_anything_at(x, y) || d->is_spiderweb(x, y)) {
        ...
        if (! tp->normal_placement_rules()) { continue; }
...
void Level::place_portals(Dungeonp d)
{
  ...
  //
  if (portal_count == 0) {
    if (pcg_random_range(0, 100) < 90) { return; }
  }
...
void Level::place_random_treasure(Dungeonp d)
{
  ...
  while (tries-- > 0) {
    ...
    if (d->is_dirt(x, y) || d->is_weapon_class_A(x, y) || d->is_weapon_class_B(x, y) || d->is_weapon_class_C(x, y)
      ...
      if (tp->is_torch()) {
        if (d->is_deep_water(x, y) || d->is_foliage(x, y)) { continue; }
      }
      ...
      if (pcg_random_range(0, 100) < 20) { t->enchant_randomly(); }

      if (t->is_sword()) {
        //
        // Double enchant swords in lakes :)
        //
        if (d->is_shallow_water(x, y)) { t->enchant_randomly(); }

        if (d->is_deep_water(x, y)) { t->enchant_randomly(); }
      } else {
        //
