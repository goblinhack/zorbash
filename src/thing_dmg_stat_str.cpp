...
int Thing::on_owner_rcv_dmg_stat_str(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, owner);
  if (! owner) {
    err("Cannot call owner_dmg_stat_str null thing");
    return damage;
  }
  ...
  auto on_owner_rcv_dmg_stat_str = on_owner_rcv_dmg_stat_str_do();
  if (std::empty(on_owner_rcv_dmg_stat_str)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_stat_str, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_stat_str call [%s] expected mod:function, got %d elems",
      on_owner_rcv_dmg_stat_str.c_str(), (int) on_owner_rcv_dmg_stat_str.size());

  return damage;
}

int Thing::on_rcv_dmg_stat_str(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);

  if (! hitter) {
    err("Cannot call dmg_stat_str null thing");
    return damage;
  }

  auto on_rcv_dmg_stat_str = on_rcv_dmg_stat_str_do();
  if (std::empty(on_rcv_dmg_stat_str)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_stat_str, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_stat_str call [%s] expected mod:function, got %d elems", on_rcv_dmg_stat_str.c_str(),
      (int) on_rcv_dmg_stat_str.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_stat_str(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_stat_str(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_stat_str(hitter, real_hitter, damage);
...
int Thing::total_dmg_for_on_attacking_dmg_stat_str(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
const std::string &Thing::on_rcv_dmg_stat_str_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_stat_str_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_stat_str_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_stat_str_do());
}
