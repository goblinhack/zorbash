...
const std::string Thing::dmg_heat_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_heat_dice_str() + modifier_to_string(enchant_count_get());
  }

  return (tp()->dmg_heat_dice_str());
}

int Thing::dmg_heat(void)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_receive_dmg_heat(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_receive_dmg_heat = on_owner_receive_dmg_heat_do();
  if (std::empty(on_owner_receive_dmg_heat)) {
    return damage;
  }

  auto t = split_tokens(on_owner_receive_dmg_heat, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_receive_dmg_heat call [%s] expected mod:function, got %d elems",
      on_owner_receive_dmg_heat.c_str(), (int) on_owner_receive_dmg_heat.size());

  return damage;
}

int Thing::on_receiving_dmg_heat(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_receiving_dmg_heat = on_receiving_dmg_heat_do();
  if (std::empty(on_receiving_dmg_heat)) {
    return damage;
  }

  auto t = split_tokens(on_receiving_dmg_heat, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_receiving_dmg_heat call [%s] expected mod:function, got %d elems", on_receiving_dmg_heat.c_str(),
      (int) on_receiving_dmg_heat.size());

  return damage;
}

int Thing::total_dmg_for_on_receiving_dmg_heat(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_receive_dmg_heat(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_receiving_dmg_heat(hitter, real_hitter, damage);
...
int Thing::dmg_heat_chance_d1000(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_heat_chance_d1000(index));
}

const std::string &Thing::on_receiving_dmg_heat_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_receiving_dmg_heat_do());
}
...
const std::string &Thing::on_owner_receive_dmg_heat_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_receive_dmg_heat_do());
}

int Tp::dmg_heat_chance_d1000(int index) const
{
  if (index >= (int) _dmg_heat_chance_d1000.size()) {
    return 0;
  }
  return _dmg_heat_chance_d1000[ index ];
}

void Tp::dmg_heat_chance_d1000_set(int index, int v)
{
  _dmg_heat_chance_d1000.resize(index + 1);
  _dmg_heat_chance_d1000[ index ] = v;
}
