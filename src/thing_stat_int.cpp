...
int Thing::stat_int_total(void)
{
  ...
  if (stat != prev) {
    prev = stat;
    dbg3("Int: with mod (%s): %d", bonus_to_string(stat_int_bonus()).c_str(), stat);
  }
  ...
  FOR_ALL_EQUIP(e)
  {
    ...
    if (iter) {
      ...
      if (stat != prev) {
        prev = stat;
        dbg3("Int: with (%s %d): %d", iter->to_short_string().c_str(), iter->stat_int_total(), stat);
      }
    }
  }
  ...
  if (stat_int_bonus()) {
    ...
    if (stat != prev) {
      prev = stat;
      dbg3("Int: with enchant %d: %d", enchant, stat);
    }
  }
