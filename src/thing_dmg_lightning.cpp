...
const Dice &Thing::dmg_lightning_dice(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_lightning_dice());
}

const std::string Thing::dmg_lightning_dice_str(void)
{
  TRACE_NO_INDENT();

  if (enchant_count_get()) {
    return tp()->dmg_lightning_dice_str() + bonus_to_string(enchant_count_get());
  }

  return (tp()->dmg_lightning_dice_str());
}

int Thing::dmg_lightning(Thingp victim)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_rcv_dmg_lightning(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, owner);
  if (! owner) {
    err("Cannot call owner_dmg_lightning null thing");
    return damage;
  }
  ...
  auto on_owner_rcv_dmg_lightning = on_owner_rcv_dmg_lightning_do();
  if (std::empty(on_owner_rcv_dmg_lightning)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_lightning, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_lightning call [%s] expected mod:function, got %d elems",
      on_owner_rcv_dmg_lightning.c_str(), (int) on_owner_rcv_dmg_lightning.size());

  return damage;
}

int Thing::on_rcv_dmg_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
    err("Cannot call dmg_lightning null thing");
    return damage;
  }

  auto on_rcv_dmg_lightning = on_rcv_dmg_lightning_do();
  if (std::empty(on_rcv_dmg_lightning)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_lightning, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_lightning call [%s] expected mod:function, got %d elems", on_rcv_dmg_lightning.c_str(),
      (int) on_rcv_dmg_lightning.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_lightning(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_lightning(hitter, real_hitter, damage);
...
int Thing::total_dmg_for_on_attacking_dmg_lightning(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
const std::string &Thing::on_rcv_dmg_lightning_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_lightning_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_lightning_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_lightning_do());
}
