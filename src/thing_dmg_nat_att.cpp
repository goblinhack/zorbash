...
const Dice &Thing::dmg_nat_att_dice(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_nat_att_dice());
}

const std::string Thing::dmg_nat_att_dice_str(void)
{
  TRACE_NO_INDENT();

  if (enchant_count_get()) {
    return tp()->dmg_nat_att_dice_str() + bonus_to_string(enchant_count_get());
  }

  return (tp()->dmg_nat_att_dice_str());
}

int Thing::dmg_nat_att(Thingp victim)
{
  TRACE_NO_INDENT();
  auto roll    = tp()->dmg_nat_att_dice().roll();
...
int Thing::on_owner_rcv_dmg_nat_att(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, owner);
  if (! owner) {
    err("Cannot call owner_dmg_nat_att null thing");
    return damage;
  }
  ...
  auto on_owner_rcv_dmg_nat_att = on_owner_rcv_dmg_nat_att_do();
  if (std::empty(on_owner_rcv_dmg_nat_att)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_nat_att, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_nat_att call [%s] expected mod:function, got %d elems", on_owner_rcv_dmg_nat_att.c_str(),
      (int) on_owner_rcv_dmg_nat_att.size());

  return damage;
}

int Thing::on_rcv_dmg_nat_att(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);

  if (! hitter) {
    err("Cannot call dmg_nat_att null thing");
    return damage;
  }

  auto on_rcv_dmg_nat_att = on_rcv_dmg_nat_att_do();
  if (std::empty(on_rcv_dmg_nat_att)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_nat_att, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_nat_att call [%s] expected mod:function, got %d elems", on_rcv_dmg_nat_att.c_str(),
      (int) on_rcv_dmg_nat_att.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_nat_att(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_nat_att(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_nat_att(hitter, real_hitter, damage);
...
int Thing::total_dmg_for_on_attacking_dmg_nat_att(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
int Thing::dmg_chance_d1000_nat_att(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_chance_d1000_nat_att(index));
}

const std::string &Thing::on_rcv_dmg_nat_att_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_nat_att_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_nat_att_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_nat_att_do());
}

int Tp::dmg_chance_d1000_nat_att(int index) const
{
  if (index >= (int) _dmg_chance_d1000_nat_att.size()) {
    return 0;
  }
  return _dmg_chance_d1000_nat_att[ index ];
}

void Tp::dmg_chance_d1000_nat_att_set(int index, int v)
{
  _dmg_chance_d1000_nat_att.resize(index + 1);
  _dmg_chance_d1000_nat_att[ index ] = v;
}
