...
int Thing::on_owner_receive_dmg_stat_att(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  auto on_owner_receive_dmg_stat_att = on_owner_receive_dmg_stat_att_do();
  if (std::empty(on_owner_receive_dmg_stat_att)) { return damage; }

  auto t = split_tokens(on_owner_receive_dmg_stat_att, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_receive_dmg_stat_att call [%s] expected mod:function, got %d elems",
      on_owner_receive_dmg_stat_att.c_str(), (int) on_owner_receive_dmg_stat_att.size());
...
int Thing::on_receiving_dmg_stat_att(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_receiving_dmg_stat_att)) { return damage; }
...
int Thing::total_dmg_for_on_receiving_dmg_stat_att(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) { damage = iter->on_owner_receive_dmg_stat_att(this, hitter, real_hitter, damage); }
  }
...
int Thing::on_attacking_dmg_stat_att(Thingp victim, int damage)
{
  ...
  if (std::empty(on_attacking_dmg_stat_att)) { return damage; }
...
int Thing::on_owner_attack_dmg_stat_att(Thingp owner, Thingp victim, int damage)
{
  ...
  if (std::empty(on_owner_attack_dmg_stat_att)) { return damage; }
...
int Thing::total_dmg_for_on_attacking_dmg_stat_att(Thingp victim, int damage)
{
  ...
  {
    auto iter = level->thing_find(item.id);
    if (iter) { damage = iter->on_owner_attack_dmg_stat_att(this, victim, damage); }
  }
...
const std::string &Thing::on_owner_receive_dmg_stat_att_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_receive_dmg_stat_att_do());
}
