...
void Level::tick_gas_healing(void)
{
      ...
      if (is_gas_blocker(x, y)) {
        for (auto dy = 0; dy < DUNGEON_GAS_RESOLUTION; dy++) {
          for (auto dx = 0; dx < DUNGEON_GAS_RESOLUTION; dx++) {
            ...
            set_no_check(gas_healing_cloud, gx, gy, (uint8_t) 255);
          }
        }
      } else {
        //
        ...
        for (auto dy = 0; dy < DUNGEON_GAS_RESOLUTION; dy++) {
          for (auto dx = 0; dx < DUNGEON_GAS_RESOLUTION; dx++) {
            uint16_t gx = x * DUNGEON_GAS_RESOLUTION + dx;
            uint16_t gy = y * DUNGEON_GAS_RESOLUTION + dy;
            if (get_no_check(gas_healing_cloud, gx, gy) == 255) {
              set_no_check(gas_healing_cloud, gx, gy, (uint8_t) 0);
            }
          }
        }
      }
  ...
  for (uint16_t y = DUNGEON_GAS_RESOLUTION; y < (MAP_HEIGHT * DUNGEON_GAS_RESOLUTION) - DUNGEON_GAS_RESOLUTION; y++) {
    ...
    for (; x < (MAP_WIDTH * DUNGEON_GAS_RESOLUTION) - DUNGEON_GAS_RESOLUTION; x++) {
      uint8_t n = gas_healing_cloud[ x ][ y ];
      if (n == 255) {
        printf("XXX");
      } else if (n) {
        printf("%3u", n);
  ...
  for (uint16_t y = DUNGEON_GAS_RESOLUTION; y < (MAP_HEIGHT * DUNGEON_GAS_RESOLUTION) - DUNGEON_GAS_RESOLUTION; y++) {
    ...
    for (uint16_t x = DUNGEON_GAS_RESOLUTION; x < (MAP_WIDTH * DUNGEON_GAS_RESOLUTION) - DUNGEON_GAS_RESOLUTION;
         x++) {
      uint8_t gn = get_no_check(gas_healing_cloud, x, y);
      ...
      uint8_t gc = get_no_check(old_gas_healing_cloud, x + 1, y - 1);
      uint8_t gd = get_no_check(old_gas_healing_cloud, x - 1, y);
      uint8_t ge = get_no_check(old_gas_healing_cloud, x, y);
      uint8_t gf = get_no_check(old_gas_healing_cloud, x + 1, y);
      uint8_t gg = get_no_check(old_gas_healing_cloud, x - 1, y + 1);
      uint8_t gh = get_no_check(old_gas_healing_cloud, x, y + 1);
      uint8_t gi = get_no_check(old_gas_healing_cloud, x + 1, y + 1);
...
void Level::gas_healing_explosion(point at)
{
  TRACE_NO_INDENT();
  ...
  for (auto dx = 0; dx < DUNGEON_GAS_RESOLUTION; dx++) {
    for (auto dy = 0; dy < DUNGEON_GAS_RESOLUTION; dy++) {
      ...
      gas_healing_cloud[ gx ][ gy ] = 0;
    }
  }
  is_gas_healing_no_check_set(at.x, at.y, 0);

  dbg("Healing gas explosion");
  TRACE_AND_INDENT();

  thing_new("explosion_major", at);

  gas_healing_explosion(at + point(1, 0));
  gas_healing_explosion(at + point(-1, 0));
  gas_healing_explosion(at + point(1, -1));
  gas_healing_explosion(at + point(-1, 1));
}
...
uint8_t Level::is_gas_healing_no_check(const point p) const { return (get_no_check(_is_gas_healing, p.x, p.y)); }
...
uint8_t Level::is_gas_healing_no_check(const int x, const int y) const
{
  return (get_no_check(_is_gas_healing, x, y));
}
...
void Level::is_gas_healing_no_check_set(const int x, const int y, uint8_t val)
{
  set_no_check(_is_gas_healing, x, y, val);
}
...
void Level::is_gas_healing_no_check_unset(const int x, const int y)
{
  set_no_check(_is_gas_healing, x, y, (uint8_t) 0);
}
