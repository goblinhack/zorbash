...
int Thing::total_on_damage_stat_str(Thingp hitter, Thingp real_hitter, int damage)
{
  FOR_ALL_BUFFS(item)
  {
    auto iter = level->thing_find(item.id);
    if (iter) {
      damage = iter->on_owner_damage_stat_str(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_damage_stat_str(hitter, real_hitter, damage);
