...
int Tp::get_danger_level(void)
{
  ...
  if (is_player()) { danger_level /= 2; }
  if (is_undead()) { danger_level++; }
  if (is_lifeless()) { danger_level++; }
  if (is_sticky()) { danger_level++; }
  if (is_spiderweb()) { danger_level += 5; }
  if (is_able_to_be_resurrected()) { danger_level *= 2; }
  if (is_able_to_regenerate()) { danger_level *= 2; }
  if (is_able_to_shove()) { danger_level++; }
  if (is_able_to_grapple()) { danger_level++; }
  if (is_able_to_rage()) { danger_level++; }
  if (is_acid()) { danger_level++; }
  if (hunger_is_insatiable()) { danger_level++; }
  if (is_fire()) { danger_level++; }
  if (is_fire_elemental()) { danger_level++; }
  if (is_cold_elemental()) { danger_level++; }
  if (is_lava()) { danger_level++; }
  if (is_poisonous_danger_level()) { danger_level += is_poisonous_danger_level(); }
  if (is_necrotic_danger_level()) { danger_level += is_necrotic_danger_level() * 20; }
  if (is_floating()) { danger_level += 2; }
  if (is_flying()) { danger_level += 2; }
  if (is_able_to_jump()) { danger_level += 5; }
...
int Thing::danger_initial_level(void)
{
  ...
  if (is_able_to_grapple()) { danger_level++; }
  if (is_able_to_rage()) { danger_level++; }
  if (is_carrying_an_item()) { danger_level++; }
  if (is_acid()) { danger_level++; }
  if (hunger_is_insatiable()) { danger_level++; }
  if (is_fire()) { danger_level++; }
  if (is_fire_elemental()) { danger_level++; }
  ...
  //
  if (health() < health_max() / 5) { danger_level /= 5; }

  if (health() < health_max() / 10) { danger_level /= 10; }
...
int Thing::danger_current_level(void)
{
  ...
  if (is_able_to_follow()) {
    if (follower_count()) { danger_level *= follower_count(); }
  }

  //
  // Low on health, reduce the level
  //
  if (health() < health_max() / 5) { danger_level /= 2; }
...
{
  if (! it->is_mob() && ! it->is_monst() && ! it->is_player()) { return false; }
  ...
  //
  if (distance(curr_at, it->curr_at) < 2) { its_danger_level += 10; }

  if (distance(curr_at, it->curr_at) == 0) { its_danger_level += 20; }
  ...
  if (is_cowardly()) { its_danger_level *= 10; }
  ...
  //
  if (is_cautious()) { its_danger_level *= 2; }

  //
  // Will consider more dangerous things less dangerous
  //
  if (is_daring()) { its_danger_level /= 2; }

  //
  // Will consider more dangerous things less dangerous
  //
  if (is_fearless()) { its_danger_level = 0; }
...
const std::string Thing::danger_level_str(Thingp it)
{
  ...
  if (delta > 20) { return "%%fg=red$Deadly. Avoid!"; }
  if (delta > 10) { return "%%fg=red$Dangerous"; }
  if (delta >= 5) { return "%%fg=orange$Caution advised"; }
  if (delta >= 0) { return "%%fg=orange$Some caution advised"; }
  if (delta >= -10) { return "%%fg=yellow$Slight caution needed"; }
  if (delta >= -20) { return "%%fg=green$Mostly harmless"; }
  if (delta >= -30) { return "%%fg=green$Harmless"; }
...
ThingDangerLevel Thing::danger_level(Thingp it)
{
  ...
  if (delta > 20) { return THING_DANGER_LEVEL_CRITICAL; }
  if (delta > 10) { return THING_DANGER_LEVEL_DANGEROUS; }
  if (delta >= 5) { return THING_DANGER_LEVEL_CAUTION_ADVISED; }
  if (delta >= 0) { return THING_DANGER_LEVEL_MODERATE_CAUTION; }
  if (delta >= -10) { return THING_DANGER_LEVEL_SLIGHT_CAUTION; }
  if (delta >= -20) { return THING_DANGER_LEVEL_MOSTLY_HARMLESS; }
  if (delta >= -30) { return THING_DANGER_LEVEL_HARMLESS; }
