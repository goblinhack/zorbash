...
{_
    ...
    // are referred to.
    //
    LOG("level fini: destroy all things");
    ...
    LOG("level fini: garbage collection of things still on the map:");
    thing_gc();

    //
    // Check all things were fully detached
    //
    LOG("level fini: check all things are detached");
    for (auto x = 0; x < MAP_WIDTH; ++x) {
        for (auto y = 0; y < MAP_HEIGHT; ++y) {
            for (auto z = 0; z < MAP_SLOTS; ++z) {
                auto id = get(all_thing_ids_at, x, y, z);
    ...
    //
    LOG("level fini: check all things ids are released");
