...
bool Thing::collision_find_best_target(ThingAttackOptionsp attack_options)
{
  ...
  for (auto &cand : thing_colls) {
    ...
    if (t == owner) {
      dbg("Collision-candidate: %s no, my thing", t->to_short_string().c_str());
      continue;
    }
    ...
    if (t->is_dead || t->is_dying) {
      if (! can_eat(t) && ! t->is_frozen) {
        dbg("Collision-candidate: %s no dead or dying", t->to_short_string().c_str());
        continue;
      }
    }

    if (is_friend(t) || same_mob(t)) {
      dbg("Collision-candidate: %s no same leader", t->to_short_string().c_str());
      continue;
    }
    ...
    if (t->immediate_owner()) {
      dbg("Collision-candidate: %s no, thing is owned", t->to_short_string().c_str());
      continue;
    }
  ...
  if ((owner_it == me) || (owner_me == it)) {
    ...
    if (is_fire()) {
      if (is_torch()) {
        ...
        // Abort the walk
        //
        dbg("No collision; don't set fire to yourself by carrying a torch");
        return false;
      }
...
bool Thing::collision_check_only(Thingp it, point future_pos)
{
  ...
  IF_DEBUG2
  {
    dbg("Collision check only? at %d,%d with %s)", future_pos.x, future_pos.y, it->to_short_string().c_str());
  }
  TRACE_AND_INDENT();
  ...
  if (is_engulfer() && can_eat(it) && (it->curr_at == future_pos)) {
    if (d1000() < me_tp->chance_d1000_attack_engulf()) {
      if (it->thing_size() < thing_size()) {
        IF_DEBUG2 { dbg("No collision; can engulf"); }
        return false;
      }
    }
  }
  ...
  if (it->is_player() || it->is_monst()) {
    ...
    if (is_barrel() || is_block_of_ice()) {
      if (things_overlap(me, future_pos, it)) {
        if (it->is_soft()) {
          IF_DEBUG2 { dbg("Overlaps; barrel can splat soft monst"); }
          return false;
        }
        if (it->is_flying()) {
          IF_DEBUG2 { dbg("Overlaps; barrel can splat flying monst"); }
          return false;
        }
        if (it->is_ethereal()) {
          IF_DEBUG2 { dbg("Overlaps; barrel can splat ethereal monst"); }
          return false;
        }
        if (it->is_crushable()) {
          IF_DEBUG2 { dbg("Overlaps; barrel can crush monst"); }
          return false;
        }
        IF_DEBUG2 { dbg("Overlaps; barrel cannot splat"); }
        return true;
      }
    } else if (is_brazier()) {
      //
      ...
      if (can_eat(it)) {
        if (things_overlap(me, me->curr_at, it)) {
          IF_DEBUG2 { dbg("Collision; overlaps and can eat"); }
          return true;
        }
      }
      ...
      if (it->is_dead && ! it->is_frozen) {
        if (! it->is_obs_when_dead()) {
          IF_DEBUG2 { dbg("No collision; ignore corpse"); }
          return false;
        }
      }
    }
  ...
  if (it->is_spell_of_holding_barrier() && ! is_immune_to_spell_of_holding()) {
    if (it->curr_at == curr_at) {
      ...
      //
      IF_DEBUG2 { dbg("Collision; inside spell barrier"); }
      return true;
    }
  }
  ...
  if (it->is_barrel() && ! is_ethereal() && ! is_flying()) {
    ...
    if (it->curr_at == curr_at) {
      ...
      //
      IF_DEBUG2 { dbg("No collision; allow movement away from barrel"); }
      return false;
    }

    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("Collision; overlaps barrel"); }
      return true;
    }
  }
  ...
  if (it->is_block_of_ice() && ! is_ethereal() && ! is_flying()) {
    if (it->curr_at == curr_at) {
      ...
      //
      IF_DEBUG2 { dbg("Collision; inside block of ice"); }
      return true;
    }
    ...
    //
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("Collision; overlaps block of ice"); }
      return true;
    }
  }
  ...
  if (it->is_brazier() && ! is_ethereal() && ! is_flying()) {
    ...
    if (it->curr_at == curr_at) {
      ...
      //
      IF_DEBUG2 { dbg("No collision; allow movement away from brazier"); }
      return false;
    }

    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("Collision; overlaps brazier"); }
      return true;
    }
  }
  ...
  //
  if ((owner_it == me) || (owner_me == it)) {
    IF_DEBUG2 { dbg("No collision; collision with myself"); }
    return false;
  }
  ...
  if (me_tp->gfx_pixelart_attack_anim()) {
    if (it_tp->is_monst()) {
      ...
      //
      if (things_overlap(me, future_pos, it)) {
        IF_DEBUG2 { dbg("Collision; overlaps and can attack monst"); }
        it->wake("bumped into");
        return true;
      }
    }
  ...
  if (it->is_door() && ! it->is_open) {
    if (! it->is_dead && ! it->is_frozen) {
      if (things_overlap(me, future_pos, it)) {
        IF_DEBUG2 { dbg("Collision; overlaps and can open"); }
        if (open_door(it)) {
          return false;
        }
        if (things_overlap(me, future_pos, it)) {
          IF_DEBUG2 { dbg("Collision; overlaps and can attack door"); }
          return true;
        }
      }
    } else {
  ...
  if (it->is_ethereal()) {
    if (things_overlap(me, future_pos, it)) {
      ...
      //
      if (is_one_per_tile() && it->is_one_per_tile()) {
        IF_DEBUG2 { dbg("Collision; Stop ethereal things from piling up"); }
        return true;
      }
      ...
      if (is_monst() || is_player()) {
        ...
        if (l && (l == this)) {
          //
          // This is really for spectral blades, so we can pass through them
          //
          IF_DEBUG2 { dbg("No collision; can pass through ethereal friends"); }
          return false;
        }

        IF_DEBUG2 { dbg("Collision; Stop monst moving on ethereal things"); }
        return true;
      }

      IF_DEBUG2 { dbg("No collision; can pass through ethereal thing"); }
      return false;
    }
  }
  ...
  if (it->is_obs_wall_or_door()) {
    if (is_able_to_walk_through_walls()) {
      if ((future_pos.x < MAP_BORDER_ROCK) || (future_pos.y < MAP_BORDER_ROCK)
        ...
        IF_DEBUG2 { dbg("No collision; can pass through walls but too close to the edge"); }
        if (is_player()) {
          msg("The wall is too thick here to move");
        }
        return true;
      }
      IF_DEBUG2 { dbg("No collision; can pass through walls"); }
      return false;
    }
  }
  ...
  if (it->is_flying()) {
    if (things_overlap(me, future_pos, it)) {
      ...
      //
      if (is_flying()) {
        IF_DEBUG2 { dbg("Collision; Stop flying things from piling up"); }
        return true;
      }
      ...
      //
      if (is_monst() || is_player()) {
        IF_DEBUG2 { dbg("Collision; Stop monst moving on flying things"); }
        return true;
      }

      IF_DEBUG2 { dbg("No collision; can pass through flying thing"); }
      return false;
    }
  }
  ...
  if (it->is_descend_dungeon()) {
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("No collision; overlaps but can exit"); }
      return false;
    }
  }
  ...
  if (it->is_ascend_sewer()) {
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("No collision; overlaps but can exit via sewer entrance"); }
      return false;
    }
  }
  ...
  if (it->is_descend_sewer()) {
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("No collision; overlaps but can exit via sewer exit"); }
      return false;
    }
  }

  if (is_player() && it->is_collectable()) {
    IF_DEBUG2 { dbg("No collision; allow manual collect instead"); }
    return false;
  }
  ...
  if (it->is_spiderweb() && ! is_ethereal() && ! is_immune_to_spiderwebs()) {
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("No collision; overlaps and is web; allow movement into"); }
      return false;
    }
  }
  ...
  if (is_player()) {
    ...
    auto l = it->leader();
    if (l && (l == level->player)) {
      IF_DEBUG2 { dbg("No collision; overlaps and is a pet"); }
      return false;
    }
  }
  ...
  if (possible_to_attack(it)) {
    if (things_overlap(me, future_pos, it)) {
      IF_DEBUG2 { dbg("Collision; overlaps and can attack it"); }
      it->wake("bumped into");
      return true;
    }
    IF_DEBUG2 { dbg("No collision; can attack but no overlap"); }
    return false;
  }

  if (can_eat(it)) {
    if (where_i_failed_to_collect_last_get() == it->curr_at) {
      IF_DEBUG2 { dbg("No collision; tried to collect previously"); }
      where_i_failed_to_collect_last_set(point(-1, -1));
      return false;
    }

    if (game->tick_current < it->tick_last_dropped() + 1) {
      IF_DEBUG2 { dbg("No collision; can eat but was seen previously"); }
      return false;
    }

    if (things_overlap(me, me->curr_at, it)) {
      IF_DEBUG2 { dbg("Collision; can eat and overlaps"); }
      return true;
    }
    IF_DEBUG2 { dbg("Collision; can eat but no overlap"); }
    return false;
  }
  ...
  if (is_ethereal()) {
    if (it->is_obs_wall_or_door()) {
      if (is_able_to_walk_through_walls()) {
        IF_DEBUG2 { dbg("No collision; I am ethereal and can walk through walls"); }
        return false;
      } else {
        IF_DEBUG2 { dbg("Collision; I am ethereal but unable to walk through walls"); }
        return true;
      }
    } else {
      IF_DEBUG2 { dbg("No collision; I am ethereal"); }
      return false;
    }
  }
  ...
  if (things_overlap(me, future_pos, it)) {
    ...
    //
    if (collision_obstacle(it)) {
      IF_DEBUG2 { dbg("Collision; overlaps and is an obstacle"); }
      return true;
    }
  }
    ...
    for (int16_t y = miny; y <= maxy; y++) {
      ...
      FOR_ALL_COLLISION_THINGS(level, it, x, y)
      {
        ...
        if (! collision_add_candidates(it, future_pos, x, y, dx, dy)) {
          IF_DEBUG2 { dbg("Collision check, abort walk"); }
          return false;
        }
      }
      TRACE_NO_INDENT();
...
bool Thing::collision_check_only(point future_pos)
{
  ...
  if (diff.x == -1) {
    if (diff.y == -1) {
      if (level->is_door(curr_at.x, curr_at.y - 1) || level->is_door(curr_at.x - 1, curr_at.y)) {
        IF_DEBUG2 { dbg("Cannot move diagonally"); }
        return true;
      }
    } else if (diff.y == 1) {
      if (level->is_door(curr_at.x, curr_at.y + 1) || level->is_door(curr_at.x - 1, curr_at.y)) {
  ...
  FOR_ALL_COLLISION_THINGS(level, it, future_pos.x, future_pos.y)
  {
    ...
    if (it->is_falling || it->is_jumping || it->is_changing_level) {
      IF_DEBUG2 { dbg("Ignore falling %s", it->to_short_string().c_str()); }
      continue;
    }

    if (collision_check_only(it, future_pos)) {
      IF_DEBUG2 { dbg("Cannot move; collision"); }
      return true;
    }
  }
  TRACE_NO_INDENT();
