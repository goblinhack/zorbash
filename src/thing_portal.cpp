...
void Thing::portal_tick(void)
{
  ...
  if (tick_last_teleported() != -1) {
    if (tick_last_teleported() >= game->tick_current - 1) { return; }
  }
  ...
  if (! is_able_to_be_teleported()) { return; }

  if (! level->is_portal(curr_at.x, curr_at.y)) { return; }
  ...
  for (auto x = MAP_BORDER_ROCK; x < MAP_WIDTH - MAP_BORDER_ROCK; x++) {
    for (auto y = MAP_BORDER_ROCK; y < MAP_HEIGHT - MAP_BORDER_ROCK; y++) {
      ...
      if (! level->is_portal(x, y)) { continue; }

      //
      // Ignore the originating portal
      //
      if ((curr_at.x == x) && (curr_at.y == y)) { continue; }
...
bool Thing::teleport_portal(Thingp portal)
{
  ...
  for (int dx = -radius; dx < radius; dx++) {
    for (int dy = -radius; dy < radius; dy++) {
      ...
      if (! level->is_portal(px, py)) { continue; }

      //
      // Ignore the originating portal
      //
      if ((portal->curr_at.x == px) && (portal->curr_at.y == py)) { continue; }
...
bool Thing::teleport_portal_find_target(point &other_end_of_portal)
{
  ...
  for (int dx = -radius; dx < radius; dx++) {
    for (int dy = -radius; dy < radius; dy++) {
      ...
      // Ignore the originating portal
      //
      if ((curr_at.x == px) && (curr_at.y == py)) { continue; }
