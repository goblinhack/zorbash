...
int Thing::on_owner_receiving_dmg_future1(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_receiving_dmg_future1 = on_owner_receiving_dmg_future1_do();
  if (std::empty(on_owner_receiving_dmg_future1)) {
    return damage;
  }

  auto t = split_tokens(on_owner_receiving_dmg_future1, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_receiving_dmg_future1 call [%s] expected mod:function, got %d elems",
      on_owner_receiving_dmg_future1.c_str(), (int) on_owner_receiving_dmg_future1.size());
...
int Thing::get_total_damage_for_on_receiving_dmg_future1(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_receiving_dmg_future1(this, hitter, real_hitter, damage);
    }
  }
...
int Thing::on_owner_attacking_dmg_future1(Thingp owner, Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, owner);
  if (! owner) {
  ...
  auto on_owner_attacking_dmg_future1 = on_owner_attacking_dmg_future1_do();
  if (std::empty(on_owner_attacking_dmg_future1)) {
    return damage;
  }

  auto t = split_tokens(on_owner_attacking_dmg_future1, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_owner_attacking_dmg_future1 call [%s] expected mod:function, got %d elems",
      on_owner_attacking_dmg_future1.c_str(), (int) on_owner_attacking_dmg_future1.size());

  return damage;
}

int Thing::get_total_damage_for_on_attacking_dmg_future1(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    auto iter = level->thing_find(item.id);
    if (iter) {
      damage = iter->on_owner_attacking_dmg_future1(this, victim, damage);
    }
  }
