...
void tp_random_init(void)
{
  ...
  for (auto &tp : tp_id_map) {
    ...
    if (tp->is_ethereal_mob()) { tp_ethereal_mob.push_back(tp); }
    if (tp->is_floor_deco()) { tp_deco.push_back(tp); }
    if (tp->is_floor()) { tp_floor.push_back(tp); }
    if (tp->is_foliage()) { tp_foliage.push_back(tp); }
    ...
    if (tp->is_green_blood()) { tp_green_blood.push_back(tp); }
    if (tp->is_green_splatter()) { tp_green_splatter.push_back(tp); }
    if (tp->is_key()) { tp_key.push_back(tp); }
    if (tp->is_magic_stone()) { tp_magic_stone.push_back(tp); }
    if (tp->is_mob()) { tp_mob.push_back(tp); }
    if (tp->is_player_bodypart_eyes()) { tp_bodypart[ BODYPART_EYES ].push_back(tp); }
    if (tp->is_player_bodypart_face()) { tp_bodypart[ BODYPART_FACE ].push_back(tp); }
    if (tp->is_player_bodypart_hair()) { tp_bodypart[ BODYPART_HAIR ].push_back(tp); }
    if (tp->is_player_bodypart_hat()) { tp_bodypart[ BODYPART_HAT ].push_back(tp); }
    if (tp->is_player_bodypart_legs()) { tp_bodypart[ BODYPART_LEGS ].push_back(tp); }
    if (tp->is_player_bodypart_torso()) { tp_bodypart[ BODYPART_TORSO ].push_back(tp); }
    if (tp->is_portal()) { tp_portal.push_back(tp); }
    if (tp->is_potion()) { tp_potion.push_back(tp); }
    if (tp->is_red_blood()) { tp_red_blood.push_back(tp); }
    if (tp->is_red_splatter()) { tp_red_splatter.push_back(tp); }
    if (tp->is_spiderweb()) { tp_spiderweb.push_back(tp); }
    if (tp->is_trap()) { tp_trap.push_back(tp); }
    ...
    if (tp->is_spell()) { tp_spells.push_back(tp); }
    if (tp->is_treasure_type()) { tp_treasure.push_back(tp); }

    if (! tp->is_minion()) {
      if (tp->is_monst_class_A()) { tp_monst_class_A.push_back(tp); }
      if (tp->is_monst_class_B()) { tp_monst_class_B.push_back(tp); }
      if (tp->is_monst_class_C()) { tp_monst_class_C.push_back(tp); }
      if (tp->is_monst_class_D()) { tp_monst_class_D.push_back(tp); }
      if (tp->is_monst_class_E()) { tp_monst_class_E.push_back(tp); }
    }
    ...
    if (tp->is_bag_item_container()) {
      ...
      if (tp->is_weapon_class_A()) { tp_item_class_A.push_back(tp); }
      if (tp->is_weapon_class_B()) { tp_item_class_B.push_back(tp); }
      if (tp->is_weapon_class_C()) { tp_item_class_C.push_back(tp); }
    } else {
      if (tp->is_treasure_class_A()) { tp_item_class_A.push_back(tp); }
      if (tp->is_treasure_class_B()) { tp_item_class_B.push_back(tp); }
      if (tp->is_treasure_class_C()) { tp_item_class_C.push_back(tp); }
      ...
      if (tp->is_weapon_class_B()) { tp_item_class_B.push_back(tp); }
      if (tp->is_weapon_class_C()) { tp_item_class_C.push_back(tp); }
      if (tp->is_weapon_class_A()) { tp_item_not_a_container_class_A.push_back(tp); }
      if (tp->is_weapon_class_B()) { tp_item_not_a_container_class_B.push_back(tp); }
      if (tp->is_weapon_class_C()) { tp_item_not_a_container_class_C.push_back(tp); }
    }

    if (tp->is_wall_dungeon()) { tp_wall_dungeon.push_back(tp); }
    if (tp->is_ring()) { tp_ring.push_back(tp); }
    if (tp->is_weapon()) { tp_weapon.push_back(tp); }

    if (tp->is_weapon()) {
      if (tp->is_weapon_class_A()) { tp_weapon_class_A.push_back(tp); }
      if (tp->is_weapon_class_B()) { tp_weapon_class_B.push_back(tp); }
      if (tp->is_weapon_class_C()) { tp_weapon_class_C.push_back(tp); }
    }

    if (tp->is_treasure_class_A()) { tp_treasure_class_A.push_back(tp); }
    if (tp->is_treasure_class_B()) { tp_treasure_class_B.push_back(tp); }
    if (tp->is_treasure_class_C()) { tp_treasure_class_C.push_back(tp); }

    if (tp->is_staff()) { tp_staff.push_back(tp); }
    if (tp->is_staff()) {
      if (tp->is_treasure_class_A()) { tp_staff_class_A.push_back(tp); }
      if (tp->is_treasure_class_B()) { tp_staff_class_B.push_back(tp); }
      if (tp->is_treasure_class_C()) { tp_staff_class_C.push_back(tp); }
    }

    if (tp->is_ring()) {
      if (tp->is_treasure_class_A()) { tp_ring_class_A.push_back(tp); }
      if (tp->is_treasure_class_B()) { tp_ring_class_B.push_back(tp); }
      if (tp->is_treasure_class_C()) { tp_ring_class_C.push_back(tp); }
    }

    if (! tp->is_minion()) {
      if (tp->is_monst() || tp->is_monst_pack()) { tp_monst_add(tp); }
    }
  }

  if (tp_ascend_dungeon.empty()) { DIE("No things for type:tp_ascend_dungeon"); }
  if (tp_ascend_sewer.empty()) { DIE("No things for type:tp_ascend_sewer"); }
  if (tp_barrel.empty()) { DIE("No things for type:tp_barrel"); }
  if (tp_bones.empty()) { DIE("No things for type:tp_bones"); }
  if (tp_brazier.empty()) { DIE("No things for type:tp_brazier"); }
  if (tp_deco.empty()) { DIE("No things for type:tp_deco"); }
  if (tp_descend_dungeon.empty()) { DIE("No things for type:tp_descend_dungeon"); }
  if (tp_descend_sewer.empty()) { DIE("No things for type:tp_descend_sewer"); }
  if (tp_dirt.empty()) { DIE("No things for type:tp_dirt"); }
  if (tp_door.empty()) { DIE("No things for type:tp_door"); }
  if (tp_ethereal_mob.empty()) { DIE("No things for type:tp_ethereal_mob"); }
  if (tp_floor.empty()) { DIE("No things for type:tp_floor"); }
  if (tp_foliage.empty()) { DIE("No things for type:tp_foliage"); }
  if (tp_food.empty()) { DIE("No things for type:tp_food"); }
  if (tp_gold.empty()) { DIE("No things for type:tp_gold"); }
  if (tp_grass_dry.empty()) { DIE("No things for type:tp_grass_dry"); }
  if (tp_grass_wet.empty()) { DIE("No things for type:tp_grass_wet"); }
  if (tp_green_blood.empty()) { DIE("No things for type:tp_green_blood"); }
  if (tp_green_splatter.empty()) { DIE("No things for type:tp_green_splatter"); }
  if (tp_item_class_A.empty()) { DIE("No things for type:tp_item_class_A"); }
  if (tp_item_class_B.empty()) { DIE("No things for type:tp_item_class_B"); }
  if (tp_item_class_C.empty()) { DIE("No things for type:tp_item_class_C"); }
  if (tp_item_not_a_container_class_A.empty()) { DIE("No things for type:tp_item_not_a_container_class_A"); }
  if (tp_item_not_a_container_class_B.empty()) { DIE("No things for type:tp_item_not_a_container_class_B"); }
  if (tp_item_not_a_container_class_C.empty()) { DIE("No things for type:tp_item_not_a_container_class_C"); }
  if (tp_key.empty()) { DIE("No things for type:tp_key"); }
  if (tp_large_ripples.empty()) { DIE("No things for type:tp_large_ripples"); }
  if (tp_magic_stone.empty()) { DIE("No things for type:tp_magic_stone"); }
  if (tp_mob.empty()) { DIE("No things for type:tp_mob"); }
  if (tp_monst_class_A.empty()) { DIE("No things for type:tp_monst_class_A"); }
  if (tp_monst_class_B.empty()) { DIE("No things for type:tp_monst_class_B"); }
  if (tp_monst_class_C.empty()) { DIE("No things for type:tp_monst_class_C"); }
  if (tp_monst_class_D.empty()) { DIE("No things for type:tp_monst_class_D"); }
  if (tp_monst_class_E.empty()) { DIE("No things for type:tp_monst_class_E"); }
  if (tp_portal.empty()) { DIE("No things for type:tp_portal"); }
  if (tp_potion.empty()) { DIE("No things for type:tp_potion"); }
  if (tp_red_blood.empty()) { DIE("No things for type:tp_red_blood"); }
  if (tp_red_splatter.empty()) { DIE("No things for type:tp_red_splatter"); }
  if (tp_ring_class_A.empty()) { DIE("No things for type:tp_ring_class_A"); }
  if (tp_ring_class_B.empty()) { DIE("No things for type:tp_ring_class_B"); }
  if (tp_ring_class_C.empty()) { DIE("No things for type:tp_ring_class_C"); }
  if (tp_ring.empty()) { DIE("No things for type:tp_ring"); }
  if (tp_rock.empty()) { DIE("No things for type:tp_rock"); }
  if (tp_secret_door.empty()) { DIE("No things for type:tp_secret_door"); }
  if (tp_sewer_wall.empty()) { DIE("No things for type:tp_sewer_wall"); }
  if (tp_skills.empty()) { DIE("No things for type:tp_skills"); }
  if (tp_small_ripples.empty()) { DIE("No things for type:tp_small_ripples"); }
  if (tp_spells.empty()) { CON("No things for type:tp_spells"); }
  if (tp_spiderweb.empty()) { DIE("No things for type:tp_spiderweb"); }
  if (tp_staff_class_A.empty()) { DIE("No things for type:tp_staff_class_A"); }
  if (tp_staff_class_B.empty()) { DIE("No things for type:tp_staff_class_B"); }
  if (tp_staff_class_C.empty()) { DIE("No things for type:tp_staff_class_C"); }
  if (tp_staff.empty()) { DIE("No things for type:tp_staff"); }
  if (tp_trap.empty()) { DIE("No things for type:tp_trap"); }
  if (tp_treasure_class_A.empty()) { DIE("No things for type:tp_treasure_class_A"); }
  if (tp_treasure_class_B.empty()) { DIE("No things for type:tp_treasure_class_B"); }
  if (tp_treasure_class_C.empty()) { DIE("No things for type:tp_treasure_class_C"); }
  if (tp_treasure.empty()) { DIE("No things for type:tp_treasure"); }
  if (tp_wall_dungeon.empty()) { DIE("No things for type:tp_wall_dungeon"); }
  if (tp_weapon_class_A.empty()) { DIE("No things for type:tp_weapon_class_A"); }
  if (tp_weapon_class_B.empty()) { DIE("No things for type:tp_weapon_class_B"); }
  if (tp_weapon_class_C.empty()) { DIE("No things for type:tp_weapon_class_C"); }
  if (tp_weapon.empty()) { DIE("No things for type:tp_weapon"); }
}
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_item_class_B.size())) { return tp_random_item_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_item_class_C.size())) { return tp_random_item_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_monst_class_B.size())) { return tp_random_monst_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_monst_class_C.size())) { return tp_random_monst_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_monst_class_D.size())) { return tp_random_monst_class_C(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_monst_class_E.size())) { return tp_random_monst_class_D(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_item_not_a_container_class_B.size())) { return tp_random_item_not_a_container_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_item_not_a_container_class_C.size())) { return tp_random_item_not_a_container_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_weapon_class_A.size())) { return tp_random_weapon(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_weapon_class_B.size())) { return tp_random_weapon_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_weapon_class_C.size())) { return tp_random_weapon_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_treasure_class_A.size())) { return tp_random_treasure(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_treasure_class_B.size())) { return tp_random_treasure_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_treasure_class_C.size())) { return tp_random_treasure_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_staff_class_A.size())) { return tp_random_staff(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_staff_class_B.size())) { return tp_random_staff_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_staff_class_C.size())) { return tp_random_staff_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_ring_class_A.size())) { return tp_random_ring(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_ring_class_B.size())) { return tp_random_ring_class_A(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_ring_class_C.size())) { return tp_random_ring_class_B(); }
...
{
  TRACE_NO_INDENT();
  if (unlikely(! tp_bodypart[ bodypart ].size())) { return nullptr; }
...
static Tpp tp_get_bodypart(const int bodypart, const std::string &which, int step)
{
  ...
  if (index < 0) { index = tp_bodypart[ bodypart ].size() - 1; }

  if (index >= tp_bodypart[ bodypart ].size()) { index = 0; }
