...
const std::string Thing::dmg_missile_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_missile_dice_str() + bonus_to_string(enchant_count_get());
  }

  return (tp()->dmg_missile_dice_str());
}

int Thing::dmg_missile(Thingp victim)
{
  TRACE_NO_INDENT();
...
int Thing::on_owner_rcv_dmg_missile(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_rcv_dmg_missile = on_owner_rcv_dmg_missile_do();
  if (std::empty(on_owner_rcv_dmg_missile)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_missile, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_missile call [%s] expected mod:function, got %d elems", on_owner_rcv_dmg_missile.c_str(),
      (int) on_owner_rcv_dmg_missile.size());

  return damage;
}

int Thing::on_rcv_dmg_missile(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_rcv_dmg_missile = on_rcv_dmg_missile_do();
  if (std::empty(on_rcv_dmg_missile)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_missile, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_missile call [%s] expected mod:function, got %d elems", on_rcv_dmg_missile.c_str(),
      (int) on_rcv_dmg_missile.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_missile(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_missile(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_missile(hitter, real_hitter, damage);
...
int Thing::dmg_chance_d1000_missile(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_chance_d1000_missile(index));
}

const std::string &Thing::on_rcv_dmg_missile_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_missile_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_missile_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_missile_do());
}

int Tp::dmg_chance_d1000_missile(int index) const
{
  if (index >= (int) _dmg_chance_d1000_missile.size()) {
    return 0;
  }
  return _dmg_chance_d1000_missile[ index ];
}

void Tp::dmg_chance_d1000_missile_set(int index, int v)
{
  _dmg_chance_d1000_missile.resize(index + 1);
  _dmg_chance_d1000_missile[ index ] = v;
}
