...
const Dice &Thing::damage_natural_dice_get(void)
{
  TRACE_NO_INDENT();
  return (tp()->damage_natural_dice_get());
}

const std::string Thing::damage_natural_dice_str_get(void)
{
  TRACE_NO_INDENT();

  if (enchant_get()) {
    return tp()->damage_natural_dice_str_get() + modifier_to_string(enchant_get());
  }

  return (tp()->damage_natural_dice_str_get());
}

int Thing::damage_natural_attack_get(void)
{
  TRACE_NO_INDENT();
  auto roll = tp()->damage_natural_dice_get().roll();
