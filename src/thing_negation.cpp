  ...
  if (is_monst()) {
    ...
    if (! _is_immune_to_negation) {
      if (_is_able_to_regenerate) {
        msg("%s loses the ability to regenerate!", capitalize_first(text_the()).c_str());
        _is_able_to_regenerate = false;
      } else if (_is_able_to_be_resurrected) {
        msg("%s loses the ability to resurrect!", capitalize_first(text_the()).c_str());
        _is_able_to_be_resurrected = false;
      } else if (_is_immune_to_cold) {
        msg("%s loses natural immunity to cold!", capitalize_first(text_the()).c_str());
        _is_immune_to_cold = false;
      } else if (_is_immune_to_fire) {
        msg("%s loses natural immunity to fire!", capitalize_first(text_the()).c_str());
        _is_immune_to_fire = false;
      } else if (_is_immune_to_acid) {
        msg("%s loses natural immunity to acid!", capitalize_first(text_the()).c_str());
        _is_immune_to_acid = false;
      } else if (_is_immune_to_poison) {
        msg("%s loses natural immunity to poison!", capitalize_first(text_the()).c_str());
        _is_immune_to_poison = false;
      } else if (_is_immune_to_electricity) {
        msg("%s loses natural immunity to electricity!", capitalize_first(text_the()).c_str());
        _is_immune_to_electricity = false;
      } else if (_is_immune_to_lightning) {
        msg("%s loses natural immunity to lightning!", capitalize_first(text_the()).c_str());
        _is_immune_to_lightning = false;
      } else if (_is_immune_to_magic_drain) {
        msg("%s loses natural immunity to magical draining!", capitalize_first(text_the()).c_str());
        _is_immune_to_magic_drain = false;
      } else if (_is_immune_to_stamina_drain) {
        msg("%s loses natural immunity to stamina drain!", capitalize_first(text_the()).c_str());
        _is_immune_to_stamina_drain = false;
      } else if (_is_immune_to_necrosis) {
        msg("%s loses natural immunity to necrosis!", capitalize_first(text_the()).c_str());
        _is_immune_to_necrosis = false;
      } else if (_is_immune_to_blinding) {
        msg("%s loses natural immunity to blinding!", capitalize_first(text_the()).c_str());
        _is_immune_to_blinding = false;
      } else if (_is_immune_to_confusion) {
        msg("%s loses natural immunity to confusion!", capitalize_first(text_the()).c_str());
        _is_immune_to_confusion = false;
      } else if (_is_immune_to_entrancement) {
        msg("%s loses natural immunity to entrancement!", capitalize_first(text_the()).c_str());
        _is_immune_to_entrancement = false;
      } else if (_is_immune_to_non_magical_weapons) {
        msg("%s loses natural immunity to non-magical-weapons!", capitalize_first(text_the()).c_str());
        _is_immune_to_non_magical_weapons = false;
      } else if (_is_immune_to_paralysis) {
        msg("%s loses natural immunity to paralysis!", capitalize_first(text_the()).c_str());
        _is_immune_to_paralysis = false;
      } else if (_is_immune_to_spell_of_beckoning) {
        msg("%s loses natural immunity to spell-of-beckoning!", capitalize_first(text_the()).c_str());
        _is_immune_to_spell_of_beckoning = false;
      } else if (_is_immune_to_spell_of_holding) {
        msg("%s loses natural immunity to spell-of-holding!", capitalize_first(text_the()).c_str());
        _is_immune_to_spell_of_holding = false;
      } else if (_is_immune_to_spell_of_repulsion) {
        msg("%s loses natural immunity to spell-of-repulsion!", capitalize_first(text_the()).c_str());
        _is_immune_to_spell_of_repulsion = false;
      } else if (_is_immune_to_spell_of_sanctuary) {
        msg("%s loses natural immunity to spell-of-sanctuary!", capitalize_first(text_the()).c_str());
        _is_immune_to_spell_of_sanctuary = false;
      } else if (_is_immune_to_spell_of_slowness) {
        msg("%s loses natural immunity to spell-of-slowness!", capitalize_first(text_the()).c_str());
        _is_immune_to_spell_of_slowness = false;
      } else if (_is_immune_to_teleport_attack) {
        msg("%s loses natural immunity to teleport-attack!", capitalize_first(text_the()).c_str());
        _is_immune_to_teleport_attack = false;
      } else if (_is_immune_to_vorpal_weapons) {
        msg("%s loses natural immunity to vorpal weapons!", capitalize_first(text_the()).c_str());
        _is_immune_to_vorpal_weapons = false;
      } else if (_is_immune_to_holy_damage) {
        msg("%s loses natural immunity to holy damage!", capitalize_first(text_the()).c_str());
    ...
    if (enchant_count_get()) {
      ...
      if (enchant_count_get()) {
        if (owner && owner->is_player()) {
          ...
          if (! owner->is_dying && ! owner->is_dying) {
            owner->msg("%s %s loses enchantment!", capitalize_first(owner->text_the(TEXT_APOSTROPHIZE)).c_str(),
                       text_long_name().c_str());
          }
        } else if (is_item()) {
          msg("%s loses enchantment!", capitalize_first(text_the()).c_str());
        }
      } else {
        if (owner && owner->is_player()) {
          if (! owner->is_dying && ! owner->is_dying) {
          ...
          if (! owner->is_dying && ! owner->is_dying) {
            owner->msg("%s %s is no longer enchanted!", capitalize_first(owner->text_the(TEXT_APOSTROPHIZE)).c_str(),
                       text_long_name().c_str());
          }
        } else if (is_item()) {
          msg("%s is no longer enchanted!", capitalize_first(text_the()).c_str());
        }
      }
      ...
      if (owner && owner->is_player()) {
        ...
        if (! owner->is_dying && ! owner->is_dying) {
          owner->msg("%s %s has no more charges!", capitalize_first(owner->text_the(TEXT_APOSTROPHIZE)).c_str(),
                     text_long_name().c_str());
          owner->drop(this);
        }
      } else if (is_item()) {
        msg("%s shakes!", capitalize_first(text_the()).c_str());
      }
    }
  }
}
