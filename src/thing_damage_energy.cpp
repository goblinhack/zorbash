...
int Thing::on_owner_damage_energy(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_damage_energy = on_owner_damage_energy_do();
  if (std::empty(on_owner_damage_energy)) {
    return damage;
  }

  auto t = split_tokens(on_owner_damage_energy, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
...
int Thing::on_damage_energy(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
...
int Thing::total_on_damage_energy(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_damage_energy(this, hitter, real_hitter, damage);
    }
  }
