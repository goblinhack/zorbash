...
const std::string Thing::dmg_bite_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_bite_dice_str() + modifier_to_string(enchant_count_get());
  }
...
int Thing::dmg_bite_chance_d1000(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_bite_chance_d1000(index));
}
...
int Tp::dmg_bite_chance_d1000(int index) const
{
  if (index >= (int) _dmg_bite_chance_d1000.size()) {
    return 0;
  }
  return _dmg_bite_chance_d1000[ index ];
}

void Tp::dmg_bite_chance_d1000_set(int index, int v)
{
  _dmg_bite_chance_d1000.resize(index + 1);
  _dmg_bite_chance_d1000[ index ] = v;
}
