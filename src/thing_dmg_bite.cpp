...
const Dice &Thing::dmg_bite_dice(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_bite_dice());
}

const std::string Thing::dmg_bite_dice_str(void)
{
  TRACE_NO_INDENT();

  if (enchant_count_get()) { return tp()->dmg_bite_dice_str() + bonus_to_string(enchant_count_get()); }
...
int Thing::on_owner_receive_dmg_bite(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (! owner) {
    err("Cannot call owner_dmg_bite null thing");
    return damage;
  }
  ...
  if (std::empty(on_owner_receive_dmg_bite)) { return damage; }
...
int Thing::on_receiving_dmg_bite(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  verify(MTYPE_THING, hitter);
  if (! hitter) {
    err("Cannot call dmg_bite null thing");
    return damage;
  }

  auto on_receiving_dmg_bite = on_receiving_dmg_bite_do();
  if (std::empty(on_receiving_dmg_bite)) { return damage; }
...
int Thing::total_dmg_for_on_receiving_dmg_bite(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) { damage = iter->on_owner_receive_dmg_bite(this, hitter, real_hitter, damage); }
  }
...
int Thing::on_attacking_dmg_bite(Thingp victim, int damage)
{
  ...
  if (std::empty(on_attacking_dmg_bite)) { return damage; }
...
int Thing::on_owner_attack_dmg_bite(Thingp owner, Thingp victim, int damage)
{
  ...
  if (std::empty(on_owner_attack_dmg_bite)) { return damage; }
...
int Thing::total_dmg_for_on_attacking_dmg_bite(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) { return damage; }
  ...
  {
    auto iter = level->thing_find(item.id);
    if (iter) { damage = iter->on_owner_attack_dmg_bite(this, victim, damage); }
  }
...
int Thing::dmg_chance_d1000_bite(int index)
{
  TRACE_NO_INDENT();
...
{
  if (index >= (int) _dmg_chance_d1000_bite.size()) { return 0; }
  return _dmg_chance_d1000_bite[ index ];
}

void Tp::dmg_chance_d1000_bite_set(int index, int v)
{
  _dmg_chance_d1000_bite.resize(index + 1);
  _dmg_chance_d1000_bite[ index ] = v;
}
