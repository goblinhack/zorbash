...
bool Thing::possible_to_attack(const Thingp victim)
{
  ...
  if (is_weapon() || is_staff() || is_ring()) {
    ...
    // If stuck in a spell barrier, we can only attack locally
    //
    if (level->is_spell_of_sanctuary_barrier(curr_at)) {
      if (victim->curr_at != curr_at) {
        if (is_debug_type()) {
          dbg("Cannot attack %s, stuck in a spell barrier", victim->to_short_string().c_str());
        }
  ...
  if (is_weapon()) {
    ...
    if (victim->is_spell_of_holding_barrier()) {
      ...
      if (is_debug_type()) {
        dbg("Can not break out of spell barrier %s", victim->to_short_string().c_str());
      }
      return false;
    }

    if (victim->is_spell_of_sanctuary_barrier()) {
    ...
    if (victim->is_block_of_ice()) {
      ...
      if (is_debug_type()) {
        dbg("Can not break out of ice %s", victim->to_short_string().c_str());
      }
      return false;
    }
  ...
  if (victim->is_alive_monst() || victim->is_combustible() || victim->is_very_combustible()
      ...
      || victim->is_spell_of_holding() || victim->is_spell_of_sanctuary() || victim->is_sticky()
...
bool Thing::attack(Thingp victim, ThingAttackOptionsp attack_options)
{
  ...
  for (int attack_num = 0; attack_num < attacks; attack_num++) {
    ...
    if (is_lightning()) {
      ...
      //
    } else if (is_player() || is_alive_monst() || is_turret() || is_totem()
