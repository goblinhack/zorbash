...
{
  if (is_monst(p)) { return true; }
  if (player && (player->curr_at == p)) { return true; }
  return false;
}
...
bool Level::is_walked(const point p) const
{
  if (unlikely(is_oob(p.x, p.y))) { return false; }
  return (get(_is_walked, p.x, p.y));
}

bool Level::is_walked_no_check(const point p) const { return (get_no_check(_is_walked, p.x, p.y)); }

bool Level::is_walked(const int x, const int y) const
{
  if (unlikely(is_oob(x, y))) { return false; }
  return (get(_is_walked, x, y));
}

bool Level::is_walked_no_check(const int x, const int y) const { return (get_no_check(_is_walked, x, y)); }

void Level::is_walked_set(const int x, const int y)
{
  if (unlikely(is_oob(x, y))) { return; }
  set(_is_walked, x, y, true);
}

void Level::is_walked_no_check_set(const int x, const int y) { set_no_check(_is_walked, x, y, true); }

void Level::is_walked_unset(const int x, const int y)
{
  if (unlikely(is_oob(x, y))) { return; }
  set(_is_walked, x, y, false);
}
