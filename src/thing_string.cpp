...
std::string Thing::to_string(void)
{
  ...
  if (! g_loading) {
    ...
    if (l && (l != this)) {
      return (string_sprintf(
          "%" PRIX32 " %s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s l:%s", id.id, tpp->name().c_str(),
          ...
          is_waiting_to_ascend_sewer ? "/asc-sewer" : "", l->to_short_string().c_str()));
    }
  }
...
std::string Thing::to_dbg_string(void)
{
  ...
  return (string_sprintf(
      ...
      is_moving ? ", is_moving" : "", is_offscreen ? ", is_offscreen" : "", is_open ? ", is_open" : "",
