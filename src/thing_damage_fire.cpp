...
int Thing::total_damage_for_on_receiving_dmg_fire(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
int Thing::on_owner_attack_dmg_fire(Thingp owner, Thingp victim, int damage)
{
  ...
  ERR("Bad on_owner_attack_dmg_fire call [%s] expected mod:function, got %d elems",
      on_owner_attack_dmg_fire.c_str(), (int) on_owner_attack_dmg_fire.size());

  return damage;
}

int Thing::total_damage_for_on_attacking_dmg_fire(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
