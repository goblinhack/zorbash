...
WidPopup *wid_thing_enchant_window;

void wid_thing_enchant_fini (void)
{_
    LOG("Thing enchant fini");
    ...
    delete wid_thing_enchant_window;
    wid_thing_enchant_window = nullptr;
}

uint8_t wid_thing_enchant_init (void)
{_
    return true;
}

void Game::wid_thing_enchant_destroy (void)
{_
    LOG("Thing enchant destroy");

    if (game->request_remake_inventory) {
        //
        // Continue
        //
    } else if (game->state == Game::STATE_ENCHANTING_ITEMS) {
        return;
    }

    wid_thing_enchant_fini();
}

void Game::wid_thing_enchant_create (const std::list<Thingp> &items)
{_
...
_
    if (game->request_remake_inventory) {
        ...
        // Continue
        //
        LOG("Remake thing enchant");
    } else if (game->state == Game::STATE_ENCHANTING_ITEMS) {
        LOG("Ignore, already enchanting items");
        return;
    }
    ...
    if (wid_thing_enchant_window) {
        LOG("Destroy window");
        wid_thing_enchant_destroy();
    }
...

    {
        ...
        bag_primary = new WidBag(temporary_bag, tl, br, "Items found");
    }
}
