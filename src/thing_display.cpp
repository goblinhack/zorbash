  ...
  if (is_monst()) {
    ...
    if (tile_curr) {
      ...
      if (tile && tile->is_invisible) {
        if (level
            && (level->is_gas_poison(curr_at.x, curr_at.y) || level->is_gas_explosive(curr_at.x, curr_at.y)
                || level->is_gas_confusion(curr_at.x, curr_at.y) || level->is_gas_paralysis(curr_at.x, curr_at.y)
                || level->is_darkness(curr_at.x, curr_at.y) || level->is_gas_healing(curr_at.x, curr_at.y))) {
