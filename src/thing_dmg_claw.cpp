...
const std::string Thing::dmg_claw_dice_str(void)
{
  ...
  if (enchant_count_get()) { return tp()->dmg_claw_dice_str() + bonus_to_string(enchant_count_get()); }
...
int Thing::on_owner_receive_dmg_claw(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_owner_receive_dmg_claw)) { return damage; }
...
int Thing::on_receiving_dmg_claw(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_receiving_dmg_claw)) { return damage; }
...
int Thing::total_dmg_for_on_receiving_dmg_claw(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) { damage = iter->on_owner_receive_dmg_claw(this, hitter, real_hitter, damage); }
  }
...
int Thing::on_attacking_dmg_claw(Thingp victim, int damage)
{
  ...
  if (std::empty(on_attacking_dmg_claw)) { return damage; }
...
int Thing::on_owner_attack_dmg_claw(Thingp owner, Thingp victim, int damage)
{
  ...
  if (std::empty(on_owner_attack_dmg_claw)) { return damage; }
...
int Thing::total_dmg_for_on_attacking_dmg_claw(Thingp victim, int damage)
{
  ...
  {
    auto iter = level->thing_find(item.id);
    if (iter) { damage = iter->on_owner_attack_dmg_claw(this, victim, damage); }
  }
...
{
  if (index >= (int) _dmg_chance_d1000_claw.size()) { return 0; }
