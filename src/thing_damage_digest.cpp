...
int Thing::on_owner_receive_dmg_digest(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (! owner) {
    err("Cannot call owner_damage_digest null thing");
    return damage;
  }
...
int Thing::on_receiving_dmg_digest(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  verify(MTYPE_THING, hitter);
  if (! hitter) {
    err("Cannot call damage_digest null thing");
    return damage;
  }
...
int Thing::total_damage_for_on_receiving_dmg_digest(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
int Thing::total_damage_for_on_attacking_dmg_digest(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
