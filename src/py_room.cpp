      ...
      for (auto &c : py_obj_to_stringing(o)) {
        ...
        if (m.is_floor || m.is_bridge || m.is_corridor || m.is_secret_corridor || m.is_dirt) {
          floor_string += c;
                   ...
                   m.is_mob_spawner_easy || m.is_mob_spawner_hard || m.is_monst_easy || m.is_monst_med ||
                   m.is_monst_hard || m.is_potion || m.is_secret_door || m.is_shallow_water || m.is_trap ||
        ...
        if (m.is_blood || m.is_ascend_dungeon || m.is_descend_dungeon || m.is_floor_deco || m.is_food ||
            m.is_mob_spawner_easy || m.is_mob_spawner_hard || m.is_gold || m.is_key || m.is_monst_easy ||
