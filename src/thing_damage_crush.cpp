...
int Thing::on_receiving_dmg_crush(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  ERR("Bad on_receiving_dmg_crush call [%s] expected mod:function, got %d elems",
      on_receiving_dmg_crush.c_str(), (int) on_receiving_dmg_crush.size());

  return damage;
}

int Thing::get_total_damage_for_on_receiving_dmg_crush(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
...
int Thing::on_attacking_dmg_crush(Thingp victim, int damage)
{
  ...
  ERR("Bad on_attacking_dmg_crush call [%s] expected mod:function, got %d elems",
      on_attacking_dmg_crush.c_str(), (int) on_attacking_dmg_crush.size());
...
int Thing::get_total_damage_for_on_attacking_dmg_crush(Thingp victim, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
