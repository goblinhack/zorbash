...
int Thing::on_owner_receiving_damage_crush(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  ERR("Bad on_owner_receiving_damage_crush call [%s] expected mod:function, got %d elems", on_owner_receiving_damage_crush.c_str(),
      (int) on_owner_receiving_damage_crush.size());
...
int Thing::on_receiving_damage_crush(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  ERR("Bad on_receiving_damage_crush call [%s] expected mod:function, got %d elems", on_receiving_damage_crush.c_str(),
      (int) on_receiving_damage_crush.size());

  return damage;
}

int Thing::total_on_receiving_damage_crush(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
