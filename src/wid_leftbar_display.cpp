                                         ...
                                         bool first = false)
{
  ...
  //
  // Hide the health of things like mimics
  //
  bool show_health_bar = false;

  if (t->gfx_pixelart_health_bar_shown()) {
    if (t->is_sleeping) {
      if (! t->gfx_pixelart_health_bar_only_when_awake()) {
  ...
  if (t->is_dead) {
    ...
    if (t->confused_count()) {
      TRACE_NO_INDENT();
      auto  w  = wid_new_square_button(wid_leftbar, "(Confused)");
      point tl = make_point(0, y_at);
      point br = make_point(width - 1, y_at);
      wid_set_pos(w, tl, br);
    ...
    if (t->entranced_count()) {
      TRACE_NO_INDENT();
      auto  w  = wid_new_square_button(wid_leftbar, "(Entranced)");
      point tl = make_point(0, y_at);
      point br = make_point(width - 1, y_at);
      wid_set_pos(w, tl, br);
    ...
    if (! t->is_player()) {
      ...
      if (l && (l == level->player)) {
        TRACE_NO_INDENT();
        auto  w  = wid_new_square_button(wid_leftbar, "(Beholden)");
        point tl = make_point(0, y_at);
        point br = make_point(width - 1, y_at);
        wid_set_pos(w, tl, br);
...

  {
    ...
    FOR_ALL_DESCRIBABLE_THINGS_ON_LEVEL(game->level, t)
    {
      if (! t->is_door() && ! t->is_ascend_sewer() && ! t->is_descend_sewer() && ! t->is_ascend_dungeon()
          ...
          && ! t->is_portal() && ! t->is_block_of_ice() && ! t->is_block_of_crystal()
          && ! t->is_spell_of_holding_barrier() && ! t->is_spell_of_sanctuary_barrier() && ! t->is_fungus()) {
        continue;
      }
