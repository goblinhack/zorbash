...
void Thing::update(void)
{
  ...
  if (has_temperature()) {
    ...
    if (unlikely(v)) { temperature_set(v); }
  }
  if (! stamina()) {
    TRACE_NO_INDENT();
    auto v = tpp->stamina();
  ...
  }
  {
    ...
    if (unlikely(v)) { distance_throw_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->enchant_max();
    if (unlikely(v)) { enchant_max_current_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->jump_distance_bonus();
    if (unlikely(v)) { jump_distance_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->lifespan();
    if (unlikely(v)) { lifespan_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->move_speed_bonus();
    if (unlikely(v)) { move_speed_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->noise_on_moving();
    if (unlikely(v)) { noise_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->shove_bonus();
    if (unlikely(v)) { shove_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_att_bonus();
    if (unlikely(v)) { stat_att_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_att();
    if (unlikely(v)) { stat_att_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_con_bonus();
    if (unlikely(v)) { stat_con_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_con();
    if (unlikely(v)) { stat_con_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_def_bonus();
    if (unlikely(v)) { stat_def_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_def();
    if (unlikely(v)) { stat_def_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_dex_bonus();
    if (unlikely(v)) { stat_dex_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_dex();
    if (unlikely(v)) { stat_dex_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_luck_bonus();
    if (unlikely(v)) { stat_luck_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_luck();
    if (unlikely(v)) { stat_luck_set(v); }
  }

  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_thv_bonus();
    if (unlikely(v)) { stat_thv_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_thv();
    if (unlikely(v)) { stat_thv_set(v); }
  }

  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_psi_bonus();
    if (unlikely(v)) { stat_psi_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_psi();
    if (unlikely(v)) { stat_psi_set(v); }
  }

  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_int_bonus();
    if (unlikely(v)) { stat_int_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_int();
    if (unlikely(v)) { stat_int_set(v); }
  }

  {
    TRACE_NO_INDENT();
    auto v = tpp->stats02();
    if (unlikely(v)) { stats02_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats03();
    if (unlikely(v)) { stats03_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats04();
    if (unlikely(v)) { stats04_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats05();
    if (unlikely(v)) { stats05_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats06();
    if (unlikely(v)) { stats06_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats07();
    if (unlikely(v)) { stats07_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stats09();
    if (unlikely(v)) { stats09_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_str_bonus();
    if (unlikely(v)) { stat_str_bonus_set(v); }
  }
  {
    TRACE_NO_INDENT();
    auto v = tpp->stat_str();
    if (unlikely(v)) { stat_str_set(v); }
  }
  ...
  if (! ts_next_frame) {
    ...
    if (gfx_pixelart_animated || gfx_ascii_animated) {
      ...
      if (tile) {
        ...
        if (is_debug_type()) { con("Tile init (random): %s", tile_name(tile).c_str()); }
      } else {
        tile_curr = 0;
      }
    }
  ...
  if (is_able_to_use_weapons()) {
    ...
    if (d1000() < chance_d1000_carrier_of_weapon_class_A()) {
      ...
      if (W) { carried += carry(W, carry_options); }
    }
  ...
  if (is_able_to_use_weapons()) {
    ...
    if (best_weapon) { equip(best_weapon, MONST_EQUIP_WEAPON); }
  }
  ...
  if (is_able_to_use_armor()) {
    ...
    if (best_armor) { equip(best_armor, MONST_EQUIP_ARMOR); }
  }
  ...
  if (is_able_to_use_staffs()) {
    ...
    if (best_staff) { equip(best_staff, MONST_EQUIP_WEAPON); }
  }
  ...
  if (is_able_to_use_rings()) {
    ...
    //
    {
      ...
      if (best_ring) { equip(best_ring, MONST_EQUIP_RING1); }
    }
    //
    // Ring 2
    //
    {
      Thingp best_ring = nullptr;
      carried_ring_highest_value(&best_ring);
      if (best_ring) { equip(best_ring, MONST_EQUIP_RING2); }
    }
  }
  ...
  if (is_able_to_use_helmet()) {
    ...
    if (best_helmet) { equip(best_helmet, MONST_EQUIP_HELMET); }
  }
  ...
  if (is_able_to_use_amulet()) {
    ...
    if (best_amulet) { equip(best_amulet, MONST_EQUIP_AMULET); }
  }
  ...
  if (is_able_to_use_boots()) {
    ...
    if (best_boots) { equip(best_boots, MONST_EQUIP_BOOTS); }
  }
  ...
  if (is_able_to_use_gauntlet()) {
    ...
    if (best_gauntlet) { equip(best_gauntlet, MONST_EQUIP_GAUNTLET); }
  }
  ...
  if (is_able_to_use_cloak()) {
    ...
    if (best_cloak) { equip(best_cloak, MONST_EQUIP_CLOAK); }
  }
  ...
  if (is_able_to_use_shield()) {
    ...
    if (best_shield) { equip(best_shield, MONST_EQUIP_SHIELD); }
  }
