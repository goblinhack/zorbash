...
int Thing::on_receiving_damage_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  ERR("Bad on_receiving_damage_lightning call [%s] expected mod:function, got %d elems", on_receiving_damage_lightning.c_str(),
      (int) on_receiving_damage_lightning.size());

  return damage;
}

int Thing::total_on_receiving_damage_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
