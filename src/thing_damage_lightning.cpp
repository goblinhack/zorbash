...
int Thing::on_owner_damage_lightning(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
...
int Thing::on_damage_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
...
int Thing::total_on_damage_lightning(Thingp hitter, Thingp real_hitter, int damage)
{
  FOR_ALL_BUFFS(item)
  {
    auto iter = level->thing_find(item.id);
    if (iter) {
      damage = iter->on_owner_damage_lightning(this, hitter, real_hitter, damage);
    }
  }
