...
int Thing::on_owner_rcv_dmg_stat_con(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_rcv_dmg_stat_con = on_owner_rcv_dmg_stat_con_do();
  if (std::empty(on_owner_rcv_dmg_stat_con)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_stat_con, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_stat_con call [%s] expected mod:function, got %d elems",
      on_owner_rcv_dmg_stat_con.c_str(), (int) on_owner_rcv_dmg_stat_con.size());

  return damage;
}

int Thing::on_rcv_dmg_stat_con(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  ...
  auto on_rcv_dmg_stat_con = on_rcv_dmg_stat_con_do();
  if (std::empty(on_rcv_dmg_stat_con)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_stat_con, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_stat_con call [%s] expected mod:function, got %d elems", on_rcv_dmg_stat_con.c_str(),
      (int) on_rcv_dmg_stat_con.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_stat_con(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_stat_con(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_stat_con(hitter, real_hitter, damage);
...
const std::string &Thing::on_rcv_dmg_stat_con_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_stat_con_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_stat_con_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_stat_con_do());
}
