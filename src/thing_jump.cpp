...
bool Thing::try_to_jump(point to, bool be_careful, bool *too_far)
{
  ...
  //
  if (level->is_spell_of_holding_barrier(curr_at) && ! is_immune_to_spell_of_holding()) {
  ...
  if (g_opt_ascii || ! is_visible_to_player) {
    ...
    //
    // Check the number of things jumping is not slowness the game too much
    //
  ...
  level->sound_sources_incr(to.x, to.y, noise_on_jumping());
...
bool Thing::jump_attack(Thingp maybe_victim)
{
  ...
  if (maybe_victim) {
    if (d1000() < tp()->chance_d1000_jump_attack()) {
