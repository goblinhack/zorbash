...
const Dice &Thing::damage_melee_dice_get(void)
{
  TRACE_NO_INDENT();
  return (tp()->damage_melee_dice_get());
}
...
const std::string Thing::damage_melee_dice_str_get(void)
{
  ...
  if (enchant_get()) {
    return tp()->damage_melee_dice_str_get() + modifier_to_string(enchant_get());
  }

  return (tp()->damage_melee_dice_str_get());
}

int Thing::damage_melee_get(void)
{
  TRACE_NO_INDENT();
  auto roll = tp()->damage_melee_dice_get().roll();
