...
const std::string Thing::dmg_energy_dice_str(void)
{
  ...
  if (enchant_count_get()) { return tp()->dmg_energy_dice_str() + bonus_to_string(enchant_count_get()); }
...
int Thing::on_owner_receive_dmg_energy(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_owner_receive_dmg_energy)) { return damage; }
...
int Thing::on_receiving_dmg_energy(Thingp hitter, Thingp real_hitter, int damage)
{
  ...
  if (std::empty(on_receiving_dmg_energy)) { return damage; }
...
int Thing::total_dmg_for_on_receiving_dmg_energy(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) { damage = iter->on_owner_receive_dmg_energy(this, hitter, real_hitter, damage); }
  }
...
int Thing::on_attacking_dmg_energy(Thingp victim, int damage)
{
  ...
  if (std::empty(on_attacking_dmg_energy)) { return damage; }
...
int Thing::on_owner_attack_dmg_energy(Thingp owner, Thingp victim, int damage)
{
  ...
  if (std::empty(on_owner_attack_dmg_energy)) { return damage; }
...
int Thing::total_dmg_for_on_attacking_dmg_energy(Thingp victim, int damage)
{
  ...
  {
    auto iter = level->thing_find(item.id);
    if (iter) { damage = iter->on_owner_attack_dmg_energy(this, victim, damage); }
  }
...
{
  if (index >= (int) _dmg_chance_d1000_energy.size()) { return 0; }
