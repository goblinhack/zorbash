...
const std::string Thing::dmg_energy_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_energy_dice_str() + modifier_to_string(enchant_count_get());
  }

  return (tp()->dmg_energy_dice_str());
}

int Thing::dmg_energy(void)
{
  TRACE_NO_INDENT();
...
int Thing::dmg_energy_chance_d1000(int index)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_energy_chance_d1000(index));
}
...
int Tp::dmg_energy_chance_d1000(int index) const
{
  if (index >= (int) _dmg_energy_chance_d1000.size()) {
    return 0;
  }
  return _dmg_energy_chance_d1000[ index ];
}

void Tp::dmg_energy_chance_d1000_set(int index, int v)
{
  _dmg_energy_chance_d1000.resize(index + 1);
  _dmg_energy_chance_d1000[ index ] = v;
}
