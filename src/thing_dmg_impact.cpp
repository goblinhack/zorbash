...
int Thing::on_owner_rcv_dmg_impact(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_rcv_dmg_impact = on_owner_rcv_dmg_impact_do();
  if (std::empty(on_owner_rcv_dmg_impact)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_impact, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_impact call [%s] expected mod:function, got %d elems", on_owner_rcv_dmg_impact.c_str(),
      (int) on_owner_rcv_dmg_impact.size());

  return damage;
}

int Thing::on_rcv_dmg_impact(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_rcv_dmg_impact = on_rcv_dmg_impact_do();
  if (std::empty(on_rcv_dmg_impact)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_impact, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_impact call [%s] expected mod:function, got %d elems", on_rcv_dmg_impact.c_str(),
      (int) on_rcv_dmg_impact.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_impact(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_impact(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_impact(hitter, real_hitter, damage);
...
const std::string &Thing::on_rcv_dmg_impact_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_impact_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_impact_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_impact_do());
}
