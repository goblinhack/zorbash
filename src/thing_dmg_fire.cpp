...
int Thing::on_owner_rcv_dmg_fire(Thingp owner, Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  ...
  auto on_owner_rcv_dmg_fire = on_owner_rcv_dmg_fire_do();
  if (std::empty(on_owner_rcv_dmg_fire)) {
    return damage;
  }

  auto t = split_tokens(on_owner_rcv_dmg_fire, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
  ...
  ERR("Bad on_owner_rcv_dmg_fire call [%s] expected mod:function, got %d elems", on_owner_rcv_dmg_fire.c_str(),
      (int) on_owner_rcv_dmg_fire.size());

  return damage;
}

int Thing::on_rcv_dmg_fire(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  verify(MTYPE_THING, hitter);
  if (! hitter) {
  ...
  auto on_rcv_dmg_fire = on_rcv_dmg_fire_do();
  if (std::empty(on_rcv_dmg_fire)) {
    return damage;
  }

  auto t = split_tokens(on_rcv_dmg_fire, '.');
  if (t.size() == 2) {
    auto        mod   = t[ 0 ];
    auto        fn    = t[ 1 ];
    std::size_t found = fn.find("()");
  ...
  ERR("Bad on_rcv_dmg_fire call [%s] expected mod:function, got %d elems", on_rcv_dmg_fire.c_str(),
      (int) on_rcv_dmg_fire.size());

  return damage;
}

int Thing::total_dmg_for_on_rcv_dmg_fire(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_rcv_dmg_fire(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_rcv_dmg_fire(hitter, real_hitter, damage);
...
const std::string &Thing::on_rcv_dmg_fire_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_rcv_dmg_fire_do());
}
...
const std::string &Thing::on_owner_rcv_dmg_fire_do(void)
{
  TRACE_NO_INDENT();
  return (tp()->on_owner_rcv_dmg_fire_do());
}

int Thing::dmg_rcv_doubled_from_fire(void)
{
  TRACE_NO_INDENT();
  return (tp()->dmg_rcv_doubled_from_fire());
}
