...
const std::string Thing::dmg_fire_dice_str(void)
{
  ...
  if (enchant_count_get()) {
    return tp()->dmg_fire_dice_str() + modifier_to_string(enchant_count_get());
  }

  return (tp()->dmg_fire_dice_str());
}

int Thing::dmg_fire(void)
{
  TRACE_NO_INDENT();
...
int Thing::chance_d1000_dmg_fire(int index)
{
  TRACE_NO_INDENT();
  return (tp()->chance_d1000_dmg_fire(index));
}
...
int Tp::chance_d1000_dmg_fire(int index) const
{
  if (index >= (int) _chance_d1000_dmg_fire.size()) {
    return 0;
  }
  return _chance_d1000_dmg_fire[ index ];
}

void Tp::chance_d1000_dmg_fire_set(int index, int v)
{
  _chance_d1000_dmg_fire.resize(index + 1);
  _chance_d1000_dmg_fire[ index ] = v;
}
