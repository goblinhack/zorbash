...
std::list< Thingp > Thing::helmet_list(void)
{
  TRACE_NO_INDENT();
  ...
  FOR_ALL_CARRYING(item)
  {
    ...
    if (t->is_bag()) {
      FOR_ALL_CARRIED_BY(t, item)
      {
        auto t = level->thing_find(item.id);
        if (unlikely(! t)) { continue; }
        if (t->is_helmet()) { out.push_back(t); }
      }
    }
    if (t->is_helmet()) { out.push_back(t); }
  }
...
std::vector< Thingp > Thing::helmet_vector(void)
{
  TRACE_NO_INDENT();
...
int Thing::carried_helmet_count(void)
{
  TRACE_NO_INDENT();
  int count = 0;
  ...
  for (const auto t : helmet_list()) {
    if (t->is_helmet()) { count++; }
  }
  return count;
}

int Thing::carried_helmet_least_value(Thingp *out)
{
  TRACE_NO_INDENT();
  int least_value = -1;
...
int Thing::carried_helmet_highest_value(Thingp *out)
{
  TRACE_NO_INDENT();
  int highest_value = -1;
...
int Thing::carried_helmet_highest_value_for_target(Thingp *out, Thingp target)
{
  TRACE_NO_INDENT();
  int highest_value = -1;
