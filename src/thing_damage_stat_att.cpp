...
int Thing::total_on_damage_stat_att(Thingp hitter, Thingp real_hitter, int damage)
{
  TRACE_NO_INDENT();
  if (! maybe_itemsp()) {
    return damage;
  ...
  FOR_ALL_BUFFS(item)
  {
    ...
    if (iter) {
      damage = iter->on_owner_damage_stat_att(this, hitter, real_hitter, damage);
    }
  }
  ...
  damage = on_damage_stat_att(hitter, real_hitter, damage);
