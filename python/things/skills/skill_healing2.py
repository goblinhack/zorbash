...
def on_tick_when_activated(owner, skill, x, y):
    ...
    if my.thing_stamina(owner) < 4:
        return False  # did nothing

    if my.thing_health(owner) >= my.thing_health_max(owner):
        return False  # did nothing

    my.thing_stamina_decr(owner, 4)
...
def tp_init(name, text_long_name, text_short_name):
    ...
    # begin sort marker
    my.skill_replaces(self, "skill_healing1")
    my.stat_con_bonus(self, 2)
    my.text_description_long2(self, "Conversion rate is 4 stamina per turn to 2 health.")
    my.text_description_very_short(self, "AccHeal 2")
