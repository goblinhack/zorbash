...
def on_tick_when_activated(owner, skill, x, y):
    ...
    if my.thing_stamina(owner) < 10:
        return False  # did nothing

    if my.thing_health(owner) >= my.thing_health_max(owner):
        return False  # did nothing

    my.thing_stamina_decr(owner, 10)
...
def tp_init(name, text_long_name, text_short_name):
    ...
    my.text_description_long3(self, "Conversion rate is 10 stamina per turn to 5 health.")
    my.text_description_very_short(self, "AccHeal 5")
    my.skill_replaces(self, "skill_healing4")
...
def init():
    tp_init(name="skill_healing5", text_long_name="accelerated health 5", text_short_name="accelerated health 5")
