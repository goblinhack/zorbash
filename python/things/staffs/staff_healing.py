...
def on_use(item, target):
    pct = 40.0 + 10.0 * my.thing_enchant_count_get(item)

    did_something = False
    old_health = my.thing_health(target)

    delta = int((my.thing_health_max(target) / 100.0) * pct)
    my.thing_health_incr(target, delta)

    new_health = my.thing_health(target)
    ...
    if did_something:
        ...
        if my.thing_is_player(target):
            my.thing_msg(target, f"The {my.thing_name_get(item)} had no effect.")
        else:
            my.thing_msg(target, f"The {my.thing_name_get(item)} had no effect on the {my.thing_name_get(target)}.")
...
def explode(me, x, y):
    ...
    if owner:
        if owner and my.thing_is_player(owner):
            my.thing_msg(me, "Your staff of healing explodes.")
        else:
            my.thing_msg(me, f"The {my.thing_name_get(owner)}'s staff of healing explodes.")
    else:
        my.thing_msg(me, "The staff of healing explodes.")
...
def tp_init(name, text_long_name, text_short_name):
    ...
    my.equip_carry_anim(self, "staff_healing_carry")
    ...
    my.gfx_targeted_laser(self, "laser_healing")
    my.gfx_targeted_radial(self, "staff_healing_radial")
    ...
    my.is_tickable(self, True)  # So it can interact with heal
...
def init():
    tp_init(name="staff_healing", text_long_name="staff of healing", text_short_name="staff of healing")
