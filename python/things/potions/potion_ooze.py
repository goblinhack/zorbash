...
def on_use(owner, item, target, x, y):
    ...
    my.spawn_at_my_position(target, "ooze")
    my.spawn_things_around_me(target, "ooze", 1)
    my.thing_hit_dmg_poison(owner, item, target, 50)
    my.thing_debuff_add(owner, "debuff_temporary_ooze")

    if owner:
        my.thing_msg(owner, "The ooze spreads...")
...
def shatters(item, target, x, y):
    ...
    if owner:
        if my.thing_is_player(owner):
            my.thing_msg(owner, "Your potion of ooze shatters. The ooze spreads...")
        else:
            my.thing_msg(owner, f"The {my.thing_name_get(owner)}'s potion of ooze shatters. The ooze spreads...")
    else:
        my.thing_msg(item, "The potion of ooze shatters. The ooze spreads...")

    my.spawn_at_my_position(item, "ooze")
    my.spawn_things_around_me(item, "ooze", 1)
